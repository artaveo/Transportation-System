export type Lang = "fa" | "en"

/**
 * Fraunces (serif) only covers Latin, so it renders Persian as tofu boxes.
 * Use the serif display face for English and Vazirmatn (sans) for Farsi.
 */
export const displayFont = (lang: Lang) => (lang === "en" ? "font-serif" : "font-sans")

export const dictionary = {
  fa: {
    dir: "rtl" as const,
    brand: "سفرِ شب‌رو",
    nav: {
      routes: "مسیرها و قیمت‌ها",
      about: "دربارهٔ ما",
      faq: "سوالات متداول",
      contact: "تماس با ما",
    },
    langButton: "English",
    login: "ورود",
    hero: {
      kicker: "حمل و نقل بین‌شهری افغانستان",
      title: "سفری آرام از دلِ کوه‌ها، از شهری به شهر دیگر",
      subtitle: "رزرو آنلاین بلیت بس‌های بزرگ بین‌شهری، در مسیر کابل تا هرات.",
      origin: "مبدأ",
      originPlaceholder: "شهر مبدأ را انتخاب کنید",
      destination: "مقصد",
      destinationPlaceholder: "شهر مقصد را انتخاب کنید",
      date: "تاریخ سفر",
      passengers: "مسافران",
      passenger: "مسافر",
      search: "جست‌وجوی سفر",
      swap: "جابه‌جایی مبدأ و مقصد",
      helper: "با انتخاب مسیر و تاریخ، نوع بس، چوکی خالی و قیمت را می‌بینید.",
    },
    routes: {
      title: "مسیرهای پرطرفدار",
      subtitle: "پرترددترین خطوط ما در مسیر جنوب‌غربی",
      from: "از",
      to: "به",
      duration: "مدت سفر",
      startingFrom: "شروع از",
      currency: "افغانی",
      hours: "ساعت",
      daily: "روزانه",
      trips: "سرویس",
      view: "مشاهدهٔ زمان‌ها",
    },
    track: {
      title: "پیگیری رزرو",
      subtitle:
        "برای مشاهدهٔ جزئیات رزرو خود، کد رهگیری و شمارهٔ تماسی که هنگام رزرو استفاده کرده‌اید را وارد کنید.",
      refLabel: "کد رهگیری",
      refPlaceholder: "مثال: SB-7K4RXT",
      phoneLabel: "شمارهٔ تماس",
      phonePlaceholder: "07xxxxxxxx",
      submit: "جست‌وجوی رزرو",
      resultTitle: "رزرو شما یافت شد",
      notFoundTitle: "رزروی با این مشخصات یافت نشد",
      notFoundBody:
        "لطفاً کد رهگیری و شمارهٔ تماس را دوباره بررسی کنید، یا با نزدیک‌ترین دفتر تماس بگیرید.",
      contactOffices: "مشاهدهٔ دفاتر",
      editContact: "ویرایش اطلاعات تماس",
      requestCancel: "درخواست کنسلی",
      editContactTitle: "ویرایش اطلاعات تماس رزرو",
      newPhoneLabel: "شمارهٔ تماس جدید",
      saveChanges: "ثبت تغییر",
      contactUpdated: "شمارهٔ تماس شما به‌روزرسانی شد. تأییدیهٔ نهایی از طریق پیامک ارسال می‌شود.",
      cancelConfirmTitle: "درخواست کنسلی رزرو",
      cancelConfirmBody:
        "درخواست کنسلی شما ثبت شد. طبق سیاست کنسلی و بازپرداخت، وضعیت نهایی از طریق شمارهٔ تماس شما اطلاع داده می‌شود.",
      cancelAction: "بله، درخواست کنسلی ثبت شود",
      cancelDismiss: "انصراف",
      viewCancellationPolicy: "مشاهدهٔ سیاست کنسلی",
      searching: "در حال جست‌وجو…",
      saving: "در حال ثبت…",
      cancelSubmitting: "در حال ثبت درخواست…",
      statusLabel: "وضعیت رزرو",
      cancelNotAllowed:
        "این رزرو قبلاً تأیید شده و از این صفحه قابل کنسلی نیست. لطفاً برای کنسلی با نزدیک‌ترین دفتر تماس بگیرید.",
      alreadyCancelled: "این رزرو قبلاً لغو شده است.",
    },
    routesPage: {
      kicker: "قیمت‌ها و زمان‌بندی",
      title: "مسیرها و قیمت‌ها",
      subtitle:
        "فهرست کامل مسیرهای فعال ما از کابل تا هرات، همراه با مدت سفر؛ برای رزرو، مسیر موردنظر را جست‌وجو کنید.",
      filterLabel: "استان مبدأ یا مقصد",
      filterAll: "همهٔ مسیرها",
      colFrom: "از",
      colTo: "به",
      colDuration: "مدت سفر",
      colPrice: "قیمت",
      colDaily: "سرویس روزانه",
      view: "جست‌وجو و رزرو",
      priceNote:
        "[PLACEHOLDER — نیاز به تأیید نهایی از شرکت] قیمت و تعداد سرویس هر مسیر از سفرهای فعلاً ثبت‌شده (داده‌ی آزمایشی) محاسبه شده‌اند، نه فهرست نهایی و تأییدشدهٔ شرکت؛ پیش از انتشار نهایی باید بازبینی شوند.",
    },
    fleet: {
      title: "ناوگانی که برای فاصله‌های طولانی ساخته شده",
      subtitle: "کوچ‌های مدل «۵۸۰» (Mercedes-Benz Travego/O580)، آمادهٔ سفرهای چندساعته میان‌شهری.",
      items: [
        {
          title: "تهویهٔ مطبوع",
          desc: "دمای مطبوع در گرمای تابستان و سرمای زمستان، در تمام طول مسیر.",
        },
        {
          title: "فضای بار مطمئن",
          desc: "جای کافی برای چمدان و بار خانواده، با نظارت در هر توقف.",
        },
        {
          title: "چوکی‌های راحت",
          desc: "چوکی‌های پهن با فضای پا برای نشستن راحت در مسیرهای طولانی.",
        },
        {
          title: "سفر به‌موقع",
          desc: "حرکت و رسیدن بر اساس جدول زمانی مشخص و قابل اعتماد.",
        },
      ],
    },
    whyUs: {
      kicker: "چرا سفرِ شب‌رو",
      title: "چرا ما را انتخاب کنید",
      subtitle: "دلایلی که مسافران مسیر کابل تا هرات به ما اعتماد می‌کنند.",
      items: [
        {
          title: "رزرو بدون حساب کاربری",
          desc: "فقط با شمارهٔ تماس رزرو کنید و کد رهگیری بگیرید؛ ساخت حساب کاربری اصلاً لازم نیست.",
          placeholder: false,
        },
        {
          title: "بلیت دیجیتال با کد QR",
          desc: "بلیت شما بلافاصله پس از پرداخت صادر می‌شود و همراه کد رهگیری و QR قابل نمایش یا چاپ است.",
          placeholder: false,
        },
        {
          title: "دو سطح خدمات، یک استاندارد آسایش",
          desc: "بسته به بودجه و نیاز سفر، میان چیدمان استاندارد ۲+۲ یا وی‌آی‌پی ۱+۲ یکی را انتخاب کنید.",
          placeholder: false,
        },
        {
          title: "پرداخت آنلاین یا نقدی",
          desc: "پرداخت آنلاین یا پرداخت نقدی در دفتر و هنگام سوار شدن — هرکدام که برایتان راحت‌تر است.",
          placeholder: false,
        },
        {
          title: "پشتیبانی در دفاتر شهری",
          desc: "دفاتر فروش در مسیر کابل تا هرات، برای رزرو حضوری یا هر پرسشی دربارهٔ سفرتان.",
          placeholder: false,
        },
        {
          title: "ردیابی زندهٔ بس",
          desc: "امکان مشاهدهٔ موقعیت بس روی نقشه در طول سفر.",
          placeholder: true,
        },
      ],
      placeholderTag: "نیاز به تأیید مشتری",
    },
    destinationsSection: {
      kicker: "شبکهٔ سفر",
      title: "مقصدهای تحت پوشش",
      subtitle: "مسیر اصلی ما شهرهای زیر را از کابل تا هرات به هم وصل می‌کند.",
      viewSchedule: "مشاهدهٔ زمان‌بندی",
      corridorNote: "همهٔ این شهرها روی یک خط اصلی قرار دارند؛ رزرو میان هر دو نقطه از این مسیر ممکن است.",
    },
    trustStats: {
      title: "سفرِ شب‌رو در یک نگاه",
      subtitle: "این اعداد پس از تأیید نهایی شرکت به‌روزرسانی می‌شوند.",
      items: [
        { label: "سال‌های فعالیت" },
        { label: "شهر تحت پوشش" },
        { label: "سرویس روزانه" },
      ],
      placeholderTag: "PLACEHOLDER",
    },
    footer: {
      tagline: "شبکهٔ حمل و نقل بین‌شهری، از کابل تا هرات.",
      officesTitle: "دفاتر شهری",
      quickTitle: "دسترسی سریع",
      contactTitle: "تماس با ما",
      phone: "شمارهٔ تماس",
      email: "ایمیل",
      rights: "تمامی حقوق محفوظ است.",
      quick: [
        { label: "مسیرها و قیمت‌ها", href: "/routes" },
        { label: "پیگیری بلیت", href: "/track" },
        { label: "سیاست بار و اثاثیه", href: "/luggage-policy" },
        { label: "سوالات متداول", href: "/faq" },
        { label: "شرایط و ضوابط", href: "/terms" },
        { label: "حریم خصوصی", href: "/privacy" },
      ],
      contactPlaceholder: "[PLACEHOLDER — نیاز به شماره و ایمیل واقعی شرکت]",
      officeDetailPlaceholder: "[PLACEHOLDER — نیاز به آدرس و شماره تماس دقیق این دفتر]",
    },
    offices: [
      { cityFa: "کابل", cityEn: "Kabul", nameFa: "دفتر مرکزی", nameEn: "Central office" },
      { cityFa: "کابل", cityEn: "Kabul", nameFa: "دفتر کوته‌سنگی", nameEn: "Kote Sangi office" },
      { cityFa: "کابل", cityEn: "Kabul", nameFa: "دفتر کمپنی", nameEn: "Kampani office" },
      { cityFa: "غزنی", cityEn: "Ghazni", nameFa: "دفتر غزنی", nameEn: "Ghazni office" },
      { cityFa: "قلات", cityEn: "Qalat", nameFa: "دفتر قلات", nameEn: "Qalat office" },
      { cityFa: "کندهار", cityEn: "Kandahar", nameFa: "دفتر اصلی", nameEn: "Main office" },
      { cityFa: "کندهار", cityEn: "Kandahar", nameFa: "دفتر باغ پل", nameEn: "Bagh-e Pol office" },
      { cityFa: "هلمند", cityEn: "Helmand", nameFa: "دفتر هلمند", nameEn: "Helmand office" },
      { cityFa: "نیمروز", cityEn: "Nimroz", nameFa: "دفتر نیمروز", nameEn: "Nimroz office" },
      { cityFa: "فراه", cityEn: "Farah", nameFa: "دفتر فراه", nameEn: "Farah office" },
      { cityFa: "هرات", cityEn: "Herat", nameFa: "دفتر هرات", nameEn: "Herat office" },
    ],
    common: {
      back: "بازگشت",
      continue: "ادامه",
      currency: "افغانی",
      total: "مجموع",
      seat: "چوکی",
      seats: "چوکی",
      passenger: "مسافر",
      passengers: "مسافر",
      to: "به",
      night: "شبانه",
      day: "روزانه",
      close: "بستن",
      optional: "اختیاری",
      placeholder: "PLACEHOLDER",
    },
    search: {
      resultsTitle: "سرویس‌های موجود",
      resultsSub: "سرویس برای این مسیر یافت شد",
      editSearch: "ویرایش جست‌وجو",
      departure: "حرکت",
      arrival: "رسیدن",
      duration: "مدت",
      seatsLeft: "چوکی خالی",
      full: "تکمیل",
      selectSeats: "انتخاب چوکی",
      sortTitle: "مرتب‌سازی",
      sortEarliest: "زودترین حرکت",
      sortCheapest: "ارزان‌ترین",
      sortFastest: "سریع‌ترین",
      filtersTitle: "پالایش",
      timeOfDay: "زمان حرکت",
      morning: "صبح",
      afternoon: "بعد از ظهر",
      evening: "شب",
      amenities: "امکانات",
      busType: "نوع بس",
      priceRange: "بازهٔ قیمت",
      upTo: "تا",
      noResults: "برای این مسیر سرویسی یافت نشد.",
      hours: "ساعت",
      flexibleDeparture: "بر اساس تکمیل ظرفیت",
    },
    amenities: {
      ac: "تهویهٔ مطبوع",
      wifi: "اینترنت",
      charging: "شارژر",
      refreshment: "پذیرایی",
      reclining: "چوکی تختخواب‌شو",
    },
    busTypes: {
      vip: "وی‌آی‌پی ۱+۲",
      standard: "استاندارد ۲+۲",
    },
    seats: {
      title: "انتخاب چوکی",
      coach: "بس",
      driver: "راننده",
      door: "درب",
      legendAvailable: "خالی",
      legendBooked: "رزروشده",
      legendSelected: "انتخاب‌شده",
      selectedTitle: "چوکی‌های انتخاب‌شده",
      none: "هنوز چوکی‌ای انتخاب نشده است.",
      max: "حداکثر تعداد چوکی انتخاب شد.",
      pricePerSeat: "قیمت هر چوکی",
      continueToCheckout: "ادامه به پرداخت",
      seatLabel: "چوکی شماره",
      statusAvailable: "خالی",
      statusBooked: "رزروشده",
      statusSelected: "انتخاب‌شده",
      layoutNote: "چیدمان این سرویس",
      holdError: "متأسفانه یکی از چوکی‌های انتخابی هم‌زمان توسط مسافر دیگری رزرو شد. نقشهٔ چوکی به‌روزرسانی شد؛ لطفاً دوباره انتخاب کنید.",
      holdErrorGeneric: "مشکلی در قفل‌کردن چوکی پیش آمد. لطفاً دوباره تلاش کنید.",
      holding: "در حال بررسی...",
    },
    checkout: {
      title: "تکمیل رزرو",
      passengerDetails: "مشخصات مسافران",
      passengerNo: "مسافر",
      fullName: "نام کامل",
      fullNamePh: "نام و تخلص",
      nationalId: "شمارهٔ تذکره",
      nationalIdPh: "شمارهٔ تذکرهٔ برقی",
      gender: "جنسیت",
      male: "مرد",
      female: "زن",
      contactTitle: "اطلاعات تماس",
      phone: "شمارهٔ تماس",
      email: "ایمیل (اختیاری)",
      luggageTitle: "بار اضافه (اختیاری)",
      luggageOption: "می‌خواهم بار اضافه ثبت کنم",
      luggageNote:
        "[PLACEHOLDER — نیاز به تعرفهٔ واقعی از شرکت] هزینهٔ دقیق بار اضافه هنوز تأیید نشده؛ در صورت انتخاب این گزینه، هماهنگی نهایی هنگام سوار شدن انجام می‌شود.",
      paymentTitle: "شیوهٔ پرداخت",
      payCard: "کارت بانکی (آنلاین)",
      payCardDesc: "پرداخت آنلاین امن با کارت بانکی از طریق درگاه HesabPay.",
      payMobile: "پول موبایلی (آنلاین)",
      payMobileDesc: "پرداخت آنلاین از طریق کیف پول موبایلی شما، از همان درگاه HesabPay.",
      payOffice: "پرداخت آفلاین",
      payOfficeDesc: "پرداخت نقدی در دفتر شرکت یا هنگام سوار شدن؛ رزرو تا زمان پرداخت «در انتظار تأیید» می‌ماند.",
      summaryTitle: "خلاصهٔ سفر",
      route: "مسیر",
      date: "تاریخ",
      seatsLabel: "چوکی‌ها",
      subtotal: "جمع بلیت‌ها",
      serviceFee: "کارمزد خدمات",
      grandTotal: "مبلغ قابل پرداخت",
      pay: "پرداخت و صدور بلیت",
      required: "تکمیل این فیلد الزامی است",
      termsLabel: "شرایط و ضوابط رزرو و همچنین سیاست حریم خصوصی را می‌پذیرم.",
      termsLink: "مشاهدهٔ شرایط و ضوابط",
      privacyLink: "حریم خصوصی",
      termsRequired: "برای ادامه باید شرایط و ضوابط را بپذیرید",
      couponTitle: "کد تخفیف (اختیاری)",
      couponPh: "مثلاً SHABRAW10",
      couponInvalid: "این کد تخفیف معتبر نیست یا منقضی شده است.",
      couponMinSeats: "این کد تخفیف فقط برای رزرو با حداقل {n} چوکی معتبر است.",
      couponMinAmount: "این کد تخفیف فقط برای خریدهای با حداقل {n} {currency} معتبر است.",
      couponRouteNotEligible: "این کد تخفیف روی این مسیر معتبر نیست.",
      couponRegisteredOnly: "این کد تخفیف فقط برای مشتریان ثبت‌نامی است؛ لطفاً وارد حساب کاربری خود شوید یا رزرو را بدون این کد ادامه دهید.",
      couponTierTooLow: "این کد تخفیف نیاز به سطح بالاتری در باشگاه مشتریان دارد.",
      couponFirstTripOnly: "این کد تخفیف فقط برای اولین سفر مشتریان تازه‌وارد است.",
      couponPerCustomerLimit: "شما قبلاً از این کد تخفیف به تعداد مجاز استفاده کرده‌اید.",
      couponAppliedAtConfirm: "کد تخفیف پس از ثبت نهایی رزرو محاسبه و در صفحهٔ تأییدیه نشان داده می‌شود.",
      onlinePendingNote:
        "درگاه پرداخت آنلاین (HesabPay) هنوز به‌طور کامل وصل نشده؛ رزرو شما با وضعیت «در انتظار تأیید» ثبت می‌شود و اپراتور به‌زودی با شما تماس می‌گیرد.",
      holdExpired: "مهلت نگه‌داشتن چوکی‌های شما به پایان رسیده. لطفاً به صفحهٔ انتخاب چوکی برگردید و دوباره انتخاب کنید.",
      genericError: "ثبت رزرو با مشکل مواجه شد. لطفاً دوباره تلاش کنید.",
      submitting: "در حال ثبت رزرو...",
    },
    confirm: {
      title: "بلیت شما صادر شد",
      subtitle: "سفر خوشی برایتان آرزومندیم. جزئیات بلیت به شمارهٔ تماس شما ارسال شد.",
      ref: "کد رهگیری",
      route: "مسیر",
      date: "تاریخ سفر",
      departure: "ساعت حرکت",
      seats: "چوکی‌ها",
      passenger: "مسافر",
      amount: "مبلغ پرداخت‌شده",
      boarding: "لطفاً ۳۰ دقیقه پیش از حرکت در ترمینال حاضر باشید و کد رهگیری را همراه داشته باشید.",
      download: "دریافت بلیت",
      addToCalendar: "افزودن به تقویم",
      home: "بازگشت به صفحهٔ اصلی",
      qrHint: "این کد QR را هنگام سوار شدن نشان دهید",
      supportTitle: "تماس اضطراری با پشتیبانی سفر",
      supportNote:
        "[PLACEHOLDER — نیاز به شمارهٔ پشتیبانی واقعی شرکت] در صورت هرگونه مشکل پیش از یا حین سفر، از طریق صفحهٔ تماس با ما یا نزدیک‌ترین دفتر با ما در ارتباط باشید.",
      contactOffices: "مشاهدهٔ دفاتر پشتیبانی",
      statusPendingOffline: "در انتظار تأیید پرداخت — لطفاً هنگام سوار شدن یا در دفتر پرداخت را نهایی کنید.",
      statusPendingOnline: "در انتظار تأیید پرداخت آنلاین — اپراتور به‌زودی با شما تماس می‌گیرد.",
    },
    about: {
      kicker: "دربارهٔ ما",
      title: "شبکه‌ای که کابل را به هرات وصل می‌کند",
      intro:
        "«سفرِ شب‌رو» مسافران را در مسیر جنوب‌غربی افغانستان — از کابل تا هرات، با توقف در غزنی، قلات، کندهار، هلمند، نیمروز و فراه — جابه‌جا می‌کند؛ با ناوگانی از کوچ‌های بزرگ بین‌شهری و برنامهٔ حرکت منظم.",
      stats: [
        { label: "سال سابقهٔ فعالیت" },
        { label: "شهر تحت پوشش" },
        { label: "بس در ناوگان" },
        { label: "سرویس روزانه" },
      ],
      statsPlaceholderNote: "[PLACEHOLDER] این آمار پس از دریافت اعداد واقعی از شرکت تکمیل می‌شود.",
      storyTitle: "داستان ما",
      storyBody:
        "شبکهٔ ما یک مسیر واحد را پوشش می‌دهد: کابل، غزنی، قلات، کندهار، هلمند، نیمروز، فراه و هرات. مسافران می‌توانند میان هر دو نقطه از این مسیر رزرو کنند.",
      safetyTitle: "تعهد به ایمنی و کیفیت",
      safetyBody:
        "هر سفر با بس‌های مدل «۵۸۰» (Mercedes-Benz Travego/O580) انجام می‌شود که برای سفرهای طولانی بین‌شهری ساخته شده‌اند. بازرسی فنی پیش از حرکت و رعایت برنامهٔ زمانی از اولویت‌های ماست.",
      fleetPhotoNote: "[PLACEHOLDER — نیاز به تصویر واقعی از دفاتر و بس‌های شرکت]",
      busTypesTitle: "دو سطح خدمات، یک استاندارد آسایش",
      busTypesIntro: "بسته به مسیر و بودجهٔ سفر، می‌توانید بین دو سطح خدمات یکی را انتخاب کنید.",
      vipDesc:
        "چیدمان ۱+۲ با فاصلهٔ بیشتر میان چوکی‌ها و ظرفیت کمتر نسبت به سرویس معمولی، تهویهٔ مطبوع، اینترنت، شارژر، و پذیرایی در طول مسیر.",
      standardDesc:
        "چیدمان ۲+۲ با چوکی‌های راحت و تهویهٔ مطبوع؛ گزینه‌ای مقرون‌به‌صرفه برای سفرهای روزمره.",
      ctaTitle: "آمادهٔ سفر بعدی هستید؟",
      ctaButton: "جست‌وجوی سفر",
    },
    contact: {
      kicker: "تماس با ما",
      title: "با ما در تماس باشید",
      subtitle:
        "برای رزرو حضوری، پیگیری بلیت، یا هر پرسش دیگر، به نزدیک‌ترین دفتر ما مراجعه کنید یا برایمان پیام بگذارید.",
      officesTitle: "دفاتر فروش",
      hours: "هر روز، ۶ صبح تا ۱۰ شب",
      hoursNote: "[PLACEHOLDER — ساعت کاری هر دفتر باید جداگانه تأیید شود]",
      mapTitle: "نقشهٔ مسیر",
      mapNote: "این نقشه، ترتیب جغرافیایی شهرهای تحت پوشش را نشان می‌دهد؛ نقشهٔ دقیق دفاتر پس از دریافت آدرس‌ها اضافه می‌شود.",
      formTitle: "ارسال پیام",
      formName: "نام شما",
      formNamePh: "نام و تخلص",
      formPhone: "شمارهٔ تماس",
      formMessage: "پیام",
      formMessagePh: "پرسش یا درخواست خود را بنویسید…",
      formSubmit: "ارسال پیام",
      required: "تکمیل این فیلد الزامی است",
      sent: "پیام شما ارسال شد. به‌زودی با شما تماس می‌گیریم.",
    },
    faq: {
      kicker: "سوالات متداول",
      title: "پرسش‌های پرتکرار",
      subtitle: "پاسخ سوالات رایج دربارهٔ رزرو، پرداخت، بار همراه، و سیاست کنسلی را اینجا بیابید.",
      groups: [
        {
          title: "رزرو",
          items: [
            {
              q: "آیا برای رزرو نیاز به ساخت حساب کاربری دارم؟",
              a: "خیر. رزرو فقط با شمارهٔ تماس انجام می‌شود و یک کد رهگیری برایتان صادر می‌گردد؛ ساخت حساب کاربری اختیاری است.",
            },
            {
              q: "حداکثر چند چوکی را می‌توانم در یک رزرو انتخاب کنم؟",
              a: "در هر رزرو تا شش چوکی قابل انتخاب است. برای گروه‌های بزرگ‌تر، لطفاً با نزدیک‌ترین دفتر تماس بگیرید.",
            },
            {
              q: "آیا می‌توانم برای شخص دیگری بلیت بخرم؟",
              a: "بله. کافی است در فرم مشخصات مسافران، نام و شمارهٔ تذکرهٔ همان شخص را وارد کنید؛ شمارهٔ تماس رزرو می‌تواند شمارهٔ خودتان باشد.",
            },
          ],
        },
        {
          title: "پرداخت",
          items: [
            {
              q: "چه روش‌های پرداختی پشتیبانی می‌شود؟",
              a: "پرداخت آنلاین با کارت بانکی یا پول موبایلی از طریق درگاه HesabPay، و همچنین پرداخت نقدی در دفتر یا هنگام سوار شدن.",
            },
            {
              q: "اگر پرداخت نقدی انتخاب کنم، بلیت من چه زمانی نهایی می‌شود؟",
              a: "رزروهای پرداخت‌آفلاین با وضعیت «در انتظار تأیید» ثبت می‌شوند و پس از تأیید پرداخت توسط اپراتور، نهایی می‌گردند.",
            },
            {
              q: "آیا پرداخت آنلاین امن است؟",
              a: "پرداخت آنلاین از طریق درگاه HesabPay انجام می‌شود و اطلاعات کارت شما مستقیماً روی سرورهای ما ذخیره نمی‌شود.",
            },
          ],
        },
        {
          title: "کنسلی و بازپرداخت",
          items: [
            {
              q: "آیا می‌توانم رزرو خود را کنسل کنم؟",
              a: "بله. کنسلی تا ۲۴ ساعت پیش از حرکت رایگان است و مبلغ به‌طور کامل بازپرداخت می‌شود. کنسلی کمتر از ۲۴ ساعت مانده به حرکت مشمول ۲۰ درصد کارمزد است.",
            },
            {
              q: "بازپرداخت چقدر زمان می‌برد؟",
              a: "بازپرداخت پرداخت‌های کارتی و موبایلی معمولاً میان سه تا هفت روز کاری انجام می‌شود. پرداخت‌های نقدی در همان دفتری که رزرو شده بازپرداخت می‌گردند.",
            },
            {
              q: "آیا می‌توانم تاریخ سفر را تغییر دهم؟",
              a: "بله، تغییر تاریخ تا ۱۲ ساعت پیش از حرکت و بدون هزینهٔ اضافه، در صورت وجود چوکی خالی در سرویس جدید امکان‌پذیر است.",
            },
          ],
        },
        {
          title: "بار و اثاثیه",
          items: [
            {
              q: "چه مقدار بار همراه مجاز است؟",
              a: "[PLACEHOLDER — نیاز به تأیید مقدار دقیق از شرکت] مقدار دقیق بار مجاز و ابعاد آن هنوز تأیید نشده؛ برای اطلاع دقیق پیش از سفر با دفتر خود تماس بگیرید.",
            },
            {
              q: "اگر بار اضافه داشته باشم چه کنم؟",
              a: "[PLACEHOLDER — نیاز به تعرفهٔ واقعی از شرکت] در فرم رزرو می‌توانید گزینهٔ «بار اضافه» را انتخاب کنید؛ هزینه و شرایط دقیق هنگام سوار شدن با شما هماهنگ می‌شود.",
            },
            {
              q: "چه اقلامی حمل آن‌ها ممنوع است؟",
              a: "مواد قابل‌اشتعال، سلاح و مهمات، و مواد خطرناک یا غیرقانونی مجاز به حمل نیستند. جزئیات کامل در صفحهٔ سیاست بار و اثاثیه آمده است.",
            },
          ],
        },
        {
          title: "در طول سفر",
          items: [
            {
              q: "چقدر زودتر باید در ترمینال حاضر باشم؟",
              a: "لطفاً حداقل ۳۰ دقیقه پیش از ساعت حرکت در ترمینال حاضر باشید و کد رهگیری را همراه داشته باشید.",
            },
            {
              q: "آیا کودکان نیاز به بلیت جداگانه دارند؟",
              a: "کودکان زیر ۲ سال روی آغوش والدین رایگان سفر می‌کنند؛ برای کودکان بالای ۲ سال بلیت کامل صادر می‌شود.",
            },
            {
              q: "آیا در طول مسیر توقف وجود دارد؟",
              a: "بله، بسته به طول مسیر، یک یا چند توقف کوتاه برای استراحت و نماز در نظر گرفته می‌شود.",
            },
          ],
        },
      ],
    },
    luggagePage: {
      kicker: "سیاست سفر",
      title: "سیاست بار و اثاثیه",
      subtitle: "مقدار مجاز بار همراه، بار اضافه، و اقلام ممنوعه پیش از سفر را اینجا بررسی کنید.",
      allowanceTitle: "بار مجاز همراه هر مسافر",
      allowanceBody: "[PLACEHOLDER — نیاز به تأیید مقدار دقیق از شرکت] وزن و ابعاد دقیق بار مجاز (چمدان در انبار بس و بار دستی) هنوز از شرکت دریافت نشده است.",
      extraTitle: "بار اضافه",
      extraBody:
        "[PLACEHOLDER — نیاز به تعرفهٔ واقعی از شرکت] در صورت داشتن بار بیش از حد مجاز، گزینهٔ «بار اضافه» را هنگام رزرو انتخاب کنید. تعرفهٔ دقیق و شرایط پذیرش، هنگام سوار شدن با اپراتور هماهنگ می‌شود.",
      prohibitedTitle: "اقلام ممنوعه",
      prohibitedItems: [
        "مواد قابل‌اشتعال یا منفجره",
        "سلاح، مهمات، و اشیای تیز بدون بسته‌بندی ایمن",
        "مواد شیمیایی خطرناک یا سمی",
        "کالای غیرقانونی طبق قوانین جمهوری اسلامی افغانستان",
      ],
      note: "این سیاست یک چارچوب کلی است و پیش از انتشار نهایی باید با اعداد و شرایط دقیق شرکت تکمیل شود.",
    },
    terms: {
      kicker: "شرایط و ضوابط",
      title: "شرایط و ضوابط استفاده",
      updatedLabel: "آخرین به‌روزرسانی",
      updatedValue: "[PLACEHOLDER]",
      notice:
        "[PLACEHOLDER — این متن یک چارچوب اولیه است و باید پیش از انتشار نهایی توسط تیم حقوقی/مدیریت شرکت بازبینی و تکمیل شود.]",
      sections: [
        {
          heading: "۱. پذیرش شرایط",
          body: "با رزرو بلیت از طریق این وبسایت، شما شرایط و ضوابط زیر را می‌پذیرید.",
        },
        {
          heading: "۲. رزرو و صدور بلیت",
          body: "رزرو با وارد کردن اطلاعات مسافر و شمارهٔ تماس معتبر انجام می‌شود. بلیت دیجیتال پس از تأیید پرداخت صادر می‌گردد.",
        },
        {
          heading: "۳. کنسلی و بازپرداخت",
          body: "سیاست کنسلی و بازپرداخت در صفحهٔ سوالات متداول توضیح داده شده و بخشی از همین شرایط محسوب می‌شود.",
        },
        {
          heading: "۴. مسئولیت بار همراه",
          body: "مسافر مسئول اظهار صحیح بار همراه و رعایت سیاست بار و اثاثیه است.",
        },
        {
          heading: "۵. تغییرات برنامهٔ سفر",
          body: "شرکت در موارد ضروری (شرایط جوی، فنی، یا امنیتی) حق تغییر یا تأخیر برنامهٔ حرکت را برای خود محفوظ می‌دارد.",
        },
      ],
    },
    privacy: {
      kicker: "حریم خصوصی",
      title: "سیاست حریم خصوصی",
      updatedLabel: "آخرین به‌روزرسانی",
      updatedValue: "[PLACEHOLDER]",
      notice:
        "[PLACEHOLDER — این متن یک چارچوب اولیه است و باید پیش از انتشار نهایی توسط تیم حقوقی/مدیریت شرکت بازبینی و تکمیل شود.]",
      sections: [
        {
          heading: "۱. اطلاعاتی که جمع‌آوری می‌کنیم",
          body: "نام مسافر، شمارهٔ تذکره، شمارهٔ تماس، و در صورت وارد کردن، ایمیل — فقط برای صدور و پیگیری بلیت.",
        },
        {
          heading: "۲. استفاده از اطلاعات",
          body: "اطلاعات شما صرفاً برای مدیریت رزرو، اطلاع‌رسانی سفر، و پشتیبانی مشتری استفاده می‌شود.",
        },
        {
          heading: "۳. اشتراک‌گذاری اطلاعات",
          body: "اطلاعات مسافران با اشخاص ثالث به‌جز درگاه پرداخت (برای پردازش تراکنش) و در صورت الزام قانونی، به اشتراک گذاشته نمی‌شود.",
        },
        {
          heading: "۴. نگهداری اطلاعات",
          body: "اطلاعات رزرو تا مدت لازم برای پیگیری و گزارش‌گیری داخلی نگهداری می‌شود.",
        },
      ],
    },
    admin: {
      title: "پنل مدیریت",
      subtitle: "نمای کلی عملیات شبکه",
      nav: {
        dashboard: "داشبورد",
        trips: "سرویس‌ها",
        bookings: "رزروها",
        buses: "ناوگان",
        routes: "مسیرها",
        cities: "شهرها",
        drivers: "رانندگان",
        reports: "گزارش‌ها",
        loyalty: "باشگاه مشتریان",
        content: "محتوای سایت",
        payments: "پرداخت‌ها",
        admins: "مدیریت ادمین‌ها",
      },
      exit: "خروج از پنل",
      stats: {
        bookings: "رزروهای امروز",
        revenue: "درآمد امروز",
        occupancy: "میانگین اشغال",
        trips: "سرویس‌های فعال",
      },
      vsYesterday: "نسبت به دیروز",
      recentTitle: "رزروهای اخیر",
      recentSub: "آخرین بلیت‌های صادرشده",
      searchPh: "جست‌وجوی کد رهگیری یا نام مسافر…",
      upcomingTitle: "سرویس‌های پیشِ‌رو",
      cols: {
        ref: "کد",
        passenger: "مسافر",
        route: "مسیر",
        date: "تاریخ",
        seats: "تعداد چوکی",
        amount: "مبلغ",
        status: "وضعیت",
        time: "حرکت",
        occupancy: "اشغال",
        bus: "بس",
        seatNumbers: "چوکی‌های انتخابی",
        registeredAt: "تاریخ ثبت",
      },
      status: {
        confirmed: "تأییدشده",
        pending: "در انتظار",
        cancelled: "لغوشده",
        completed: "تکمیل‌شده",
        refunded: "بازپرداخت‌شده",
      },
      allStatuses: "همهٔ وضعیت‌ها",
      manage: {
        addNew: "افزودن",
        edit: "ویرایش",
        delete: "حذف",
        save: "ذخیره",
        cancel: "انصراف",
        confirmDeleteTitle: "حذف شود؟",
        genericError: "مشکلی پیش آمد. لطفاً دوباره تلاش کنید.",
        active: "فعال",
        inactive: "غیرفعال",
        optional: "اختیاری",
      },
      routes: {
        title: "مدیریت مسیرها",
        subtitle: "مسیرهای فعال بین شهرهای کریدور",
        addRoute: "مسیر جدید",
        editRoute: "ویرایش مسیر",
        origin: "شهر مبدأ",
        destination: "شهر مقصد",
        distanceKm: "فاصله (کیلومتر)",
        durationMinutes: "مدت‌زمان تخمینی (دقیقه)",
        isActive: "این مسیر فعال است",
        empty: "هنوز مسیری ثبت نشده.",
        sameCityError: "مبدأ و مقصد نباید یکسان باشند.",
        duplicateError: "این مسیر (با همین مبدأ و مقصد) قبلاً ثبت شده است.",
        deleteConfirmBody: "این مسیر برای همیشه حذف می‌شود. این عمل قابل بازگشت نیست.",
        deleteInUseError: "این مسیر دارای سفر ثبت‌شده است و قابل حذف نیست؛ به‌جای حذف، آن را غیرفعال کنید.",
      },
      cities: {
        title: "مدیریت شهرها و ولایت‌ها",
        subtitleCount: "{active} فعال از مجموع {total} ولایت",
        searchPlaceholder: "جست‌وجوی نام شهر…",
        nameFa: "نام (دری)",
        nameEn: "نام (انگلیسی)",
        empty: "شهری یافت نشد.",
      },
      siteContent: {
        title: "محتوای سایت",
        subtitle:
          "تماس شرکت، آمار «درباره ما»، عکس واقعی ناوگان و دفاتر — این‌ها روی صفحات پابلیک (فوتر، صفحهٔ اصلی، درباره ما، تماس با ما) به‌جای placeholder فعلی نمایش داده می‌شوند.",
        settingsSectionTitle: "تنظیمات سراسری",
        companyPhone: "شمارهٔ تماس شرکت",
        companyPhonePlaceholder: "مثال: +93 70 000 0000",
        companyEmail: "ایمیل شرکت",
        companyEmailPlaceholder: "info@example.com",
        statsSectionTitle: "آمار «درباره ما»",
        statsSectionSubtitle:
          "روی صفحهٔ اصلی و صفحهٔ درباره ما نمایش داده می‌شوند. هر فیلد را خالی بگذارید تا همان placeholder فعلی («—») برایش باقی بماند.",
        yearsActive: "سال سابقهٔ فعالیت",
        citiesCovered: "شهر تحت پوشش",
        busesInFleet: "تعداد بس در ناوگان",
        dailyTrips: "سرویس روزانه",
        fleetPhotoSectionTitle: "عکس واقعی ناوگان",
        fleetPhotoSectionSubtitle:
          "در بخش «تعهد به ایمنی و کیفیت» صفحهٔ درباره ما، به‌جای یادداشت placeholder نمایش داده می‌شود.",
        fleetPhotoUpload: "آپلود عکس",
        fleetPhotoReplace: "تغییر عکس",
        fleetPhotoRemove: "حذف عکس",
        fleetPhotoUploading: "در حال آپلود…",
        fleetPhotoNone: "هنوز عکسی آپلود نشده.",
        fleetPhotoTooLarge: "حجم عکس نباید بیشتر از ۵ مگابایت باشد.",
        fleetPhotoInvalidType: "فقط فرمت‌های PNG، JPEG یا WebP مجاز است.",
        saveSettings: "ذخیرهٔ تنظیمات",
        settingsSaved: "تنظیمات ذخیره شد.",
        officesSectionTitle: "دفاتر",
        officesSectionSubtitle: "آدرس، تلفن و ساعت کاری هر دفتر — مستقل از بقیهٔ دفاتر.",
        addOffice: "دفتر جدید",
        editOffice: "ویرایش دفتر",
        officeCity: "شهر",
        officeNameFa: "نام دفتر (دری)",
        officeNameEn: "نام دفتر (انگلیسی)",
        officeAddressFa: "آدرس (دری)",
        officeAddressEn: "آدرس (انگلیسی)",
        officePhone: "شمارهٔ تماس دفتر",
        officeHoursFa: "ساعت کاری (دری)",
        officeHoursEn: "ساعت کاری (انگلیسی)",
        officeIsActive: "این دفتر روی سایت نمایش داده شود",
        officeEmpty: "هنوز دفتری ثبت نشده.",
        officeDeleteConfirmBody: "این دفتر برای همیشه حذف می‌شود. این عمل قابل بازگشت نیست.",
        colOffice: "دفتر",
        colCity: "شهر",
        colPhone: "تلفن",
      },
      responsiveImages: {
        title: "عکس‌های چندبرشی",
        subtitle:
          "هر بخش پس‌زمینه‌ای، ۴ نسخهٔ جداگانه برای موبایل/تبلت/دسکتاپ/واید دارد — هرکدام را جدا آپلود و کراپ کنید. تا وقتی چیزی آپلود نشود، عکس فعلی سایت همان‌جا باقی می‌ماند.",
        upload: "آپلود",
        replace: "تعویض",
        reset: "بازگشت به پیش‌فرض",
        resetConfirmTitle: "بازگشت به عکس پیش‌فرض؟",
        resetConfirmBody: "این برش دوباره همان عکس فعلی سایت را نشان می‌دهد؛ فایل آپلودشده حذف نمی‌شود ولی دیگر استفاده نمی‌شود.",
        defaultBadge: "پیش‌فرض سایت",
        customBadge: "آپلودشدهٔ ادمین",
        cropTitle: "برش عکس",
        cropHint: "با کشیدن جابه‌جا کنید و با اسلایدر بزرگ‌نمایی را تنظیم کنید — قاب دقیقاً همان نسبتی است که روی سایت نمایش داده می‌شود.",
        zoomLabel: "بزرگ‌نمایی",
        apply: "اعمال و آپلود",
        uploading: "در حال آپلود…",
        invalidType: "فقط فرمت‌های PNG، JPEG یا WebP مجاز است.",
        tooLarge: "حجم عکس نباید بیشتر از ۱۵ مگابایت باشد.",
      },
      buses: {
        title: "مدیریت ناوگان",
        subtitle: "بس‌های شرکت",
        addBus: "بس جدید",
        editBus: "ویرایش بس",
        code: "کد بس",
        plateNumber: "شمارهٔ پلاک",
        totalSeats: "ظرفیت (چوکی)",
        status: "وضعیت",
        empty: "هنوز بسی ثبت نشده.",
        duplicateCodeError: "این کد بس قبلاً استفاده شده است.",
        deleteConfirmBody: "این بس برای همیشه حذف می‌شود. سفرهای قبلی این بس دست‌نخورده می‌مانند.",
      },
      busStatus: {
        active: "فعال",
        maintenance: "تعمیراتی",
        retired: "بازنشسته",
      },
      drivers: {
        title: "مدیریت رانندگان",
        subtitle: "رانندگان شرکت",
        addDriver: "راننده جدید",
        editDriver: "ویرایش راننده",
        fullName: "نام کامل",
        phone: "شمارهٔ تماس",
        licenseNumber: "شمارهٔ جواز رانندگی",
        status: "وضعیت",
        empty: "هنوز راننده‌ای ثبت نشده.",
        duplicatePhoneError: "این شمارهٔ تماس قبلاً برای راننده‌ای دیگر ثبت شده است.",
        deleteConfirmBody: "این راننده برای همیشه حذف می‌شود.",
      },
      driverStatus: {
        active: "فعال",
        inactive: "غیرفعال",
      },
      scheduler: {
        title: "زمان‌بندی سفرها",
        subtitle: "تعریف سفر جدید و مدیریت سفرهای موجود",
        addTrip: "سفر جدید",
        editTrip: "ویرایش سفر",
        route: "مسیر",
        selectRoute: "یک مسیر انتخاب کنید",
        bus: "بس",
        noBusYet: "بدون بس (بعداً تعیین شود)",
        driver: "راننده",
        noDriverYet: "بدون راننده (بعداً تعیین شود)",
        serviceDate: "تاریخ سفر",
        scheduleType: "نوع حرکت",
        fixedTime: "ساعت مشخص",
        fillAndGo: "بر اساس تکمیل ظرفیت",
        departureTime: "ساعت حرکت",
        pricePerSeat: "قیمت هر چوکی",
        seatLayout: "چیدمان چوکی",
        totalSeats: "تعداد کل چوکی",
        status: "وضعیت سفر",
        empty: "هنوز سفری تعریف نشده.",
        noRoutesWarning: "ابتدا باید حداقل یک مسیر فعال بسازید (بخش «مسیرها»).",
        deleteConfirmBody: "این سفر برای همیشه حذف می‌شود.",
        deleteHasBookingsError: "این سفر دارای رزرو است و قابل حذف نیست؛ به‌جای حذف، وضعیت آن را به «لغوشده» تغییر دهید.",
        seatsLockedNote: "بعد از ساخت سفر، تعداد و چیدمان چوکی دیگر قابل تغییر نیست.",
        filters: {
          all: "همه",
          today: "امروز",
          scheduled: "پیش‌رو",
          departed: "حرکت‌کرده",
          completed: "رسیده",
          cancelled: "لغوشده",
        },
        capacity: "ظرفیت",
        delayed: "تأخیر احتمالی",
        markDeparted: "حرکت کرد",
        markArrived: "رسید به مقصد",
        cancelTrip: "لغو سفر",
        cancelTripConfirmTitle: "لغو این سفر؟",
        cancelTripConfirmBodyNoBookings: "این سفر هیچ رزرو فعالی ندارد و بدون مشکل لغو می‌شود.",
        cancelTripConfirmBodyWithBookings: "این سفر {count} رزرو فعال دارد. لغو سفر، خودِ رزروها را کنسل نمی‌کند — باید جداگانه با مسافران تماس گرفته شود.",
        departedAt: "زمان حرکت",
        arrivedAt: "زمان رسیدن",
      },
      tripStatus: {
        scheduled: "برنامه‌ریزی‌شده",
        boarding: "سوارشدن",
        departed: "حرکت‌کرده",
        completed: "تکمیل‌شده",
        cancelled: "لغوشده",
      },
      paymentStatus: {
        pending: "در انتظار",
        confirmed: "تأییدشده",
        failed: "ناموفق",
        refunded: "بازپرداخت‌شده",
      },
      bookingsPanel: {
        title: "مدیریت رزروها",
        subtitle: "همهٔ رزروهای ثبت‌شده روی سفرهای واقعی",
        empty: "هنوز رزروی ثبت نشده.",
        contact: "مسافر/تماس",
        paymentMethod: "روش پرداخت",
        paymentStatus: "وضعیت پرداخت",
        online: "آنلاین",
        offline: "آفلاین",
        guest: "مهمان",
        confirmPayment: "تأیید پرداخت",
        confirmPaymentConfirmTitle: "پرداخت تأیید شود؟",
        confirmPaymentConfirmBody: "این پرداخت آفلاین به‌عنوان دریافت‌شده ثبت می‌شود و رزرو تأیید می‌شود.",
        cancelBooking: "لغو رزرو",
        cancelBookingConfirmTitle: "رزرو لغو شود؟",
        cancelBookingConfirmBody: "این رزرو لغو می‌شود و چوکی‌های آن به‌حالت خالی برمی‌گردند.",
        refundPayment: "بازپرداخت",
        refundPaymentConfirmTitle: "این پرداخت بازپرداخت شود؟",
        refundPaymentConfirmBody: "چوکی‌های این رزرو به‌حالت خالی برمی‌گردند و رزرو به «بازپرداخت‌شده» تغییر می‌کند. این عمل قابل بازگشت نیست.",
        refundAmountLabel: "مبلغ بازپرداخت",
        refundFullAmountHint: "مبلغ کامل پرداخت",
        refundReasonLabel: "دلیل بازپرداخت",
        refundInvalidAmount: "مبلغ باید بین صفر تا مبلغ کامل پرداخت باشد.",
      },
      paymentsPanel: {
        title: "تاریخچهٔ پرداخت‌ها",
        subtitle: "هر تغییر وضعیت پرداخت، از ثبت رزرو تا تأیید و بازپرداخت",
        empty: "هنوز رویدادی ثبت نشده.",
        colTransition: "تغییر وضعیت",
        colAmount: "مبلغ",
        colActor: "انجام‌دهنده",
        colNote: "یادداشت",
        newPayment: "ثبت اولیه",
        actorAdmin: "ادمین",
        actorCustomer: "مسافر",
        actorSystem: "سیستم",
      },
      reportsPanel: {
        title: "گزارش‌گیری آماری",
        subtitle: "درآمد، تعداد مسافر و نرخ اشغال صندلی، به تفکیک مسیر و تاریخ سفر",
        from: "از تاریخ",
        to: "تا تاریخ",
        last7: "۷ روز گذشته",
        last30: "۳۰ روز گذشته",
        last90: "۹۰ روز گذشته",
        allTime: "همهٔ بازه‌ها",
        routeFilter: "مسیر",
        allRoutes: "همهٔ مسیرها",
        totalRevenue: "مجموع درآمد",
        totalPassengers: "مجموع مسافران",
        avgOccupancy: "میانگین اشغال",
        totalTrips: "تعداد سفرها",
        byRouteTitle: "تفکیک بر اساس مسیر",
        byDateTitle: "تفکیک بر اساس تاریخ",
        exportCsv: "خروجی CSV",
        colRoute: "مسیر",
        colDate: "تاریخ",
        colTrips: "سفرها",
        colPassengers: "مسافران",
        colRevenue: "درآمد",
        colOccupancy: "اشغال",
        empty: "هیچ سفری در این بازهٔ زمانی یافت نشد.",
        invalidRange: "تاریخ «از» باید قبل یا برابر تاریخ «تا» باشد.",
      },
      loyaltyPanel: {
        title: "مدیریت باشگاه مشتریان",
        subtitle: "آستانه و درصد تخفیف سطوح، پاداش کد معرفی، و کدهای تخفیف فصلی",
        tiersTitle: "سطوح عضویت",
        tiersSubtitle: "آستانهٔ سفر و درصد تخفیف هر سطح — قابل‌تغییر بدون نیاز به تغییر کد",
        colTier: "سطح",
        colThreshold: "آستانه (سفر تکمیل‌شده)",
        colDiscount: "درصد تخفیف",
        colTierStatus: "وضعیت",
        editTier: "ویرایش سطح",
        thresholdLabel: "حداقل سفر تکمیل‌شده",
        discountLabel: "درصد تخفیف",
        tierActiveLabel: "این سطح فعال است",
        thresholdRangeError: "این مقدار باید صفر یا بیشتر باشد.",
        discountRangeError: "درصد تخفیف باید بین ۰ تا ۱۰۰ باشد.",
        nonIncreasingWarning: "هشدار: این آستانه نسبت به سطح پایین‌تر یا بالاتر منطقی نیست؛ ترتیب طبیعی سطوح ممکن است به‌هم بخورد.",
        referralTitle: "پاداش کد معرفی",
        referralSubtitle:
          "این مبلغ بعد از تکمیل اولین سفر مشتریِ دعوت‌شده، به کیف‌پول هر دو طرف (دعوت‌کننده و دعوت‌شونده) واریز می‌شود.",
        referralAmountLabel: "مبلغ پاداش (برای هر طرف)",
        referralSaved: "ذخیره شد.",
        couponsTitle: "کدهای تخفیف",
        couponsSubtitle: "کمپین‌های فصلی، مستقل از تخفیف سطح عضویت",
        addCoupon: "کوپن جدید",
        editCoupon: "ویرایش کوپن",
        code: "کد کوپن",
        codeHelper: "به‌صورت خودکار با حروف بزرگ لاتین ذخیره می‌شود.",
        discountType: "نوع تخفیف",
        discountTypePercent: "درصدی",
        discountTypeFixed: "مبلغ ثابت",
        discountValue: "مقدار تخفیف",
        stackableWithTier: "قابل جمع‌شدن با تخفیف سطح عضویت",
        usageLimit: "سقف تعداد استفاده",
        usageLimitHelper: "خالی بگذارید برای بدون‌محدودیت.",
        usedCount: "تعداد استفاده",
        unlimited: "نامحدود",
        validFrom: "معتبر از",
        validTo: "معتبر تا",
        noDateLimit: "—",
        couponActive: "این کوپن فعال است",
        yes: "بله",
        no: "خیر",
        empty: "هنوز کوپنی ثبت نشده.",
        duplicateCodeError: "این کد کوپن قبلاً استفاده شده است.",
        invalidDiscountValue: "مقدار تخفیف باید بزرگ‌تر از صفر باشد.",
        invalidPercentValue: "درصد تخفیف باید بین ۱ تا ۱۰۰ باشد.",
        invalidDateRangeError: "تاریخ «معتبر تا» باید بعد یا برابر تاریخ «معتبر از» باشد.",
        deleteConfirmBody: "این کوپن برای همیشه حذف می‌شود.",
        deleteUsedError:
          "این کوپن قبلاً استفاده شده است و حذف آن سابقهٔ رزروهای مرتبط را از بین می‌برد؛ به‌جای حذف، آن را غیرفعال کنید.",
        advancedRulesTitle: "قوانین پیشرفته (اختیاری — خالی/بدون‌تیک یعنی بدون محدودیت)",
        minLoyaltyTier: "حداقل سطح باشگاه مشتریان",
        noTierRestriction: "بدون محدودیت سطح",
        perCustomerLimit: "سقف استفاده به‌ازای هر مشتری",
        minSeats: "حداقل تعداد چوکی",
        minAmount: "حداقل مبلغ سفارش",
        applicableRoutes: "محدود به مسیرهای خاص",
        applicableRoutesHelper: "هیچ‌کدام تیک نخورد یعنی روی همهٔ مسیرها معتبر است.",
        noRoutesYet: "هنوز مسیری ثبت نشده.",
        firstTripOnly: "فقط برای اولین سفر مشتری",
        registeredOnly: "فقط مشتری ثبت‌نامی (نه رزرو مهمان)",
      },
      admins: {
        title: "مدیریت ادمین‌ها",
        subtitle: "نقش و دسترسی هر ادمین را مشخص کنید — فقط سوپرادمین این تب را می‌بیند.",
        addAdmin: "دعوت ادمین جدید",
        colName: "نام",
        colEmail: "ایمیل",
        colRole: "نقش",
        colSections: "دسترسی‌ها",
        colStatus: "وضعیت",
        colLastSignIn: "آخرین ورود",
        neverSignedIn: "هنوز وارد نشده",
        youLabel: "(خودتان)",
        roleSuperAdmin: "سوپرادمین",
        roleLimitedAdmin: "ادمین محدود",
        allSectionsLabel: "همهٔ بخش‌ها",
        noSectionsLabel: "بدون دسترسی",
        empty: "هنوز ادمینی ثبت نشده.",
        inviteTitle: "دعوت ادمین جدید",
        editTitle: "ویرایش ادمین",
        fullNameLabel: "نام کامل",
        emailLabel: "ایمیل",
        emailHelper: "یک ایمیل دعوت برای تعیین رمز عبور برای همین آدرس ارسال می‌شود.",
        roleLabel: "نقش",
        sectionsLabel: "دسترسی به بخش‌ها",
        sectionsHelper: "سوپرادمین به همهٔ بخش‌ها دسترسی کامل دارد و نیازی به انتخاب نیست.",
        inviteSubmit: "ارسال دعوت",
        inviteSending: "در حال ارسال...",
        selfRoleLocked: "برای جلوگیری از قفل‌شدن حساب، نمی‌توانید نقش خودتان را تغییر دهید.",
        selfDeactivateLocked: "برای جلوگیری از قفل‌شدن حساب، نمی‌توانید حساب خودتان را غیرفعال کنید.",
        errorMissingFullName: "نام کامل را وارد کنید.",
        errorInvalidEmail: "ایمیل معتبر نیست.",
        errorEmailTaken: "این ایمیل قبلاً به‌عنوان یک حساب (ادمین یا مسافر) ثبت شده است.",
        errorEmailIsCustomer: "این ایمیل مال یک حساب مسافر است — یک ایمیل جدا برای ادمین استفاده کنید.",
        errorInviteFailed: "ارسال دعوت با مشکل مواجه شد. لطفاً دوباره تلاش کنید.",
        errorSelfRoleChange: "نمی‌توانید نقش حساب خودتان را تغییر دهید — از یک سوپرادمین دیگر بخواهید.",
        errorSelfDeactivate: "نمی‌توانید حساب خودتان را غیرفعال کنید — از یک سوپرادمین دیگر بخواهید.",
        managerBadge: "مدیر کل",
        managerScopeNotice:
          "شما «مدیر کل» هستید: می‌توانید ادمین‌های محدود را بسازید/ویرایش/غیرفعال کنید، ولی به حساب‌های سوپرادمین دسترسی ندارید و نمی‌توانید کسی (حتی خودتان) را سوپرادمین یا مدیر کل کنید.",
        canManageAdminsLabel: "این حساب می‌تواند ادمین‌ها را مدیریت کند (مدیر کل)",
        canManageAdminsHelper: "فقط سوپرادمین می‌تواند این را بدهد/بگیرد. کسی با این پرچم می‌تواند ادمین‌های محدود دیگر را بسازد/ویرایش کند، ولی نمی‌تواند سوپرادمین بسازد یا این پرچم را به کسی بدهد.",
        roleFixedLimitedNote: "شما فقط می‌توانید ادمین محدود بسازید.",
        errorManagerCannotTouchSuperAdmin: "شما اجازهٔ دسترسی به حساب‌های سوپرادمین را ندارید.",
        errorManagerCannotGrantSuperAdmin: "فقط سوپرادمین می‌تواند کسی را سوپرادمین کند.",
        errorManagerCannotGrantManageFlag: "فقط سوپرادمین می‌تواند صلاحیت «مدیر کل» را به کسی بدهد.",
        sections: {
          routes: "مسیرها و شهرها",
          fleet: "ناوگان (بس‌ها و رانندگان)",
          trips: "سرویس‌ها",
          bookings: "رزروها (داشبورد و گزارش‌ها هم همین‌جا)",
          payments: "پرداخت‌ها",
          loyalty: "باشگاه مشتریان",
          customers: "مشتریان",
          content: "محتوای سایت (تماس، دفاتر، آمار)",
        },
        auditTitle: "تاریخچهٔ تغییرات دسترسی",
        auditToggleShow: "نمایش تاریخچه",
        auditToggleHide: "پنهان‌کردن تاریخچه",
        auditEmpty: "هنوز تغییری ثبت نشده.",
        auditActor: "توسط",
        auditChange: {
          created: "ایجاد شد",
          role_changed: "نقش تغییر کرد",
          sections_changed: "دسترسی‌ها تغییر کرد",
          activated: "فعال شد",
          deactivated: "غیرفعال شد",
          renamed: "نام تغییر کرد",
          manage_flag_changed: "صلاحیت مدیر کل تغییر کرد",
        },
      },
    },
    account: {
      loginTitle: "ورود به حساب مسافر",
      loginSubtitle: "با ایمیل و رمز عبور خود وارد شوید تا به تاریخچهٔ رزرو، کیف‌پول و کد معرفی دسترسی داشته باشید.",
      signupTitle: "ساخت حساب مسافر",
      signupSubtitle: "با ثبت‌نام وارد باشگاه مشتریان می‌شوید: تخفیف سطح، کیف‌پول، و کد معرفی دوستان — کاملاً اختیاری، رزرو مهمان همچنان در دسترس است.",
      emailLabel: "ایمیل",
      passwordLabel: "رمز عبور",
      fullNameLabel: "نام و تخلص (اختیاری)",
      phoneLabel: "شمارهٔ تماس",
      phonePlaceholder: "07xxxxxxxx",
      referralCodeLabel: "کد معرفی (اختیاری)",
      loginSubmit: "ورود",
      forgotPasswordLink: "رمز عبور را فراموش کرده‌اید؟",
      signupSubmit: "ثبت‌نام",
      noAccountYet: "هنوز حساب ندارید؟",
      goSignup: "ثبت‌نام کنید",
      haveAccount: "قبلاً حساب ساخته‌اید؟",
      goLogin: "وارد شوید",
      confirmEmailNotice: "یک ایمیل تأیید برایتان ارسال شد. بعد از تأیید، دوباره وارد شوید تا پروفایل باشگاه مشتریان شما تکمیل شود.",
      completeProfileTitle: "تکمیل پروفایل",
      completeProfileSubtitle: "یک قدم مانده — برای فعال‌سازی باشگاه مشتریان، شمارهٔ تماس خود را وارد کنید.",
      completeProfileSubmit: "تکمیل ثبت‌نام",
      required: "تکمیل این فیلد الزامی است",
      errorInvalidCredentials: "ایمیل یا رمز عبور اشتباه است.",
      errorEmailInUse: "این ایمیل قبلاً ثبت شده است.",
      errorWeakPassword: "رمز عبور باید حداقل ۶ کاراکتر باشد.",
      errorPhoneTaken: "این شمارهٔ تماس قبلاً برای یک حساب دیگر ثبت شده است.",
      errorInvalidReferral: "کد معرفی وارد‌شده معتبر نیست.",
      errorGeneric: "مشکلی پیش آمد. لطفاً دوباره تلاش کنید.",
      backToLogin: "بازگشت به صفحهٔ ورود",
      forgotPasswordTitle: "بازیابی رمز عبور",
      forgotPasswordSubtitle: "ایمیل حساب خود را وارد کنید؛ اگر ثبت شده باشد، لینک بازیابی رمز عبور برایتان ارسال می‌شود.",
      forgotPasswordSubmit: "ارسال لینک بازیابی",
      forgotPasswordSuccessTitle: "ایمیل ارسال شد",
      forgotPasswordSuccessBody: "اگر این ایمیل در سیستم ثبت باشد، طی چند دقیقه یک لینک بازیابی رمز عبور دریافت خواهید کرد. پوشهٔ اسپم را هم بررسی کنید.",
      resetPasswordTitle: "تعیین رمز عبور جدید",
      resetPasswordSubtitle: "یک رمز عبور جدید برای حساب خود انتخاب کنید.",
      newPasswordLabel: "رمز عبور جدید",
      confirmPasswordLabel: "تکرار رمز عبور جدید",
      resetPasswordSubmit: "ذخیرهٔ رمز عبور جدید",
      resetPasswordSuccessTitle: "رمز عبور تغییر کرد",
      resetPasswordSuccessBody: "رمز عبور شما با موفقیت به‌روزرسانی شد.",
      goToDashboard: "رفتن به حساب من",
      passwordMismatch: "رمز عبور و تکرار آن یکسان نیستند.",
      resetLinkInvalidTitle: "لینک نامعتبر یا منقضی‌شده",
      resetLinkInvalidBody: "این لینک بازیابی رمز عبور دیگر معتبر نیست یا قبلاً استفاده شده است.",
      requestNewLink: "درخواست لینک تازه",
      dashboardTitle: "حساب من",
      logout: "خروج از حساب",
      tierTitle: "سطح باشگاه مشتریان",
      tripsCompletedLabel: "سفر تکمیل‌شده",
      tierDiscountLabel: "تخفیف دائمی شما",
      tierMaxReached: "شما در بالاترین سطح باشگاه مشتریان هستید.",
      tripsToNextTierPrefix: "فقط",
      tripsToNextTierSuffix: "سفر دیگر تا سطح",
      walletTitle: "کیف‌پول",
      walletBalanceLabel: "موجودی فعلی",
      walletEmpty: "هنوز تراکنشی در کیف‌پول شما ثبت نشده است.",
      walletTxType: {
        cashback: "کش‌بک",
        referral_bonus: "پاداش معرفی",
        redeemed: "استفاده در رزرو",
        manual_adjustment: "اصلاح دستی",
      },
      referralTitle: "کد معرفی دوستان",
      referralSubtitle: "این کد را با دوستانتان به اشتراک بگذارید؛ با اولین سفر تکمیل‌شدهٔ آن‌ها، هر دوی شما پاداش می‌گیرید.",
      referralCopy: "کپی کد",
      referralCopied: "کپی شد!",
      referralCompletedCountLabel: "تعداد معرفیِ موفق",
      bookingHistoryTitle: "تاریخچهٔ رزروها",
      bookingHistoryEmpty: "هنوز رزروی از این حساب ثبت نشده است.",
      startBooking: "جست‌وجوی سفر",
      viewBookingLink: "مشاهدهٔ جزئیات",
    },
  },
  en: {
    dir: "ltr" as const,
    brand: "Shabraw",
    nav: {
      routes: "Routes & prices",
      about: "About us",
      faq: "FAQ",
      contact: "Contact us",
    },
    langButton: "دری",
    login: "Sign in",
    hero: {
      kicker: "Intercity transport across Afghanistan",
      title: "A calm journey through the mountains, from one city to the next",
      subtitle: "Book online tickets for large intercity coaches, on the Kabul–Herat corridor.",
      origin: "From",
      originPlaceholder: "Select departure city",
      destination: "To",
      destinationPlaceholder: "Select destination city",
      date: "Travel date",
      passengers: "Passengers",
      passenger: "passenger",
      search: "Search trips",
      swap: "Swap origin and destination",
      helper: "Choose a route and date to see bus type, seats, and price.",
    },
    routes: {
      title: "Popular routes",
      subtitle: "Our busiest lines along the southwest corridor",
      from: "From",
      to: "To",
      duration: "Duration",
      startingFrom: "From",
      currency: "AFN",
      hours: "h",
      daily: "daily",
      trips: "trips",
      view: "View schedules",
    },
    track: {
      title: "Track your booking",
      subtitle:
        "Enter your booking reference and the phone number you used when booking to view your ticket details.",
      refLabel: "Booking reference",
      refPlaceholder: "e.g. SB-7K4RXT",
      phoneLabel: "Phone number",
      phonePlaceholder: "07xxxxxxxx",
      submit: "Find my booking",
      resultTitle: "Booking found",
      notFoundTitle: "No booking found",
      notFoundBody:
        "Please double-check your booking reference and phone number, or contact your nearest office.",
      contactOffices: "View offices",
      editContact: "Edit contact info",
      requestCancel: "Request cancellation",
      editContactTitle: "Edit booking contact info",
      newPhoneLabel: "New phone number",
      saveChanges: "Save changes",
      contactUpdated: "Your phone number has been updated. Final confirmation is sent by SMS.",
      cancelConfirmTitle: "Cancellation request",
      cancelConfirmBody:
        "Your cancellation request has been submitted. Under our cancellation and refund policy, the final status will be sent to your phone number.",
      cancelAction: "Yes, submit cancellation request",
      cancelDismiss: "Dismiss",
      viewCancellationPolicy: "View cancellation policy",
      searching: "Searching…",
      saving: "Saving…",
      cancelSubmitting: "Submitting…",
      statusLabel: "Booking status",
      cancelNotAllowed:
        "This booking has already been confirmed and can't be cancelled from this page. Please contact your nearest office to cancel it.",
      alreadyCancelled: "This booking has already been cancelled.",
    },
    routesPage: {
      kicker: "Prices and schedules",
      title: "Routes and prices",
      subtitle:
        "The full list of our active routes along the Kabul-to-Herat corridor, with journey time. Search a route to book.",
      filterLabel: "Origin or destination province",
      filterAll: "All routes",
      colFrom: "From",
      colTo: "To",
      colDuration: "Duration",
      colPrice: "Price",
      colDaily: "Daily trips",
      view: "Search & book",
      priceNote:
        "[PLACEHOLDER — needs final confirmation from the company] Price and trip counts per route are calculated from currently scheduled (demo) trips, not the company's final confirmed list; they must be reviewed before launch.",
    },
    fleet: {
      title: "A fleet built for the long road",
      subtitle: "Model \"580\" coaches (Mercedes-Benz Travego/O580), built for multi-hour intercity journeys.",
      items: [
        {
          title: "Climate control",
          desc: "Comfortable temperatures through summer heat and winter cold, the whole way.",
        },
        {
          title: "Secure luggage space",
          desc: "Room for family baggage, watched over at every stop along the route.",
        },
        {
          title: "Comfortable seats",
          desc: "Wide seats with legroom made for sitting easy on long journeys.",
        },
        {
          title: "On-time travel",
          desc: "Departures and arrivals on a clear, dependable schedule.",
        },
      ],
    },
    whyUs: {
      kicker: "Why Shabraw",
      title: "Why choose us",
      subtitle: "Why travellers on the Kabul-to-Herat corridor trust us.",
      items: [
        {
          title: "No account needed",
          desc: "Book with just a phone number and get a booking reference — no account required.",
          placeholder: false,
        },
        {
          title: "Digital ticket with QR code",
          desc: "Your ticket is issued right after payment, with a reference code and QR you can show or print.",
          placeholder: false,
        },
        {
          title: "Two service levels, one standard of comfort",
          desc: "Choose between our 2+2 standard layout or 1+2 VIP layout, depending on budget and needs.",
          placeholder: false,
        },
        {
          title: "Pay online or in cash",
          desc: "Pay online, or in cash at the office or when boarding — whichever suits you.",
          placeholder: false,
        },
        {
          title: "Support at city offices",
          desc: "Sales offices along the Kabul-to-Herat corridor for in-person booking or any question about your trip.",
          placeholder: false,
        },
        {
          title: "Live bus tracking",
          desc: "See your coach's live location on a map during the journey.",
          placeholder: true,
        },
      ],
      placeholderTag: "Needs client confirmation",
    },
    destinationsSection: {
      kicker: "Our network",
      title: "Destinations we cover",
      subtitle: "Our main corridor connects the following cities from Kabul to Herat.",
      viewSchedule: "View schedule",
      corridorNote: "All of these cities sit along one main line; booking between any two points on this route is possible.",
    },
    trustStats: {
      title: "Shabraw at a glance",
      subtitle: "These figures will be updated once confirmed by the company.",
      items: [
        { label: "Years in operation" },
        { label: "Cities served" },
        { label: "Daily departures" },
      ],
      placeholderTag: "PLACEHOLDER",
    },
    footer: {
      tagline: "An intercity transport network, Kabul to Herat.",
      officesTitle: "City offices",
      quickTitle: "Quick links",
      contactTitle: "Contact",
      phone: "Phone",
      email: "Email",
      rights: "All rights reserved.",
      quick: [
        { label: "Routes & prices", href: "/routes" },
        { label: "Track a ticket", href: "/track" },
        { label: "Luggage policy", href: "/luggage-policy" },
        { label: "FAQ", href: "/faq" },
        { label: "Terms & conditions", href: "/terms" },
        { label: "Privacy policy", href: "/privacy" },
      ],
      contactPlaceholder: "[PLACEHOLDER — needs the company's real phone number and email]",
      officeDetailPlaceholder: "[PLACEHOLDER — needs this office's exact address and phone number]",
    },
    offices: [
      { cityFa: "کابل", cityEn: "Kabul", nameFa: "دفتر مرکزی", nameEn: "Central office" },
      { cityFa: "کابل", cityEn: "Kabul", nameFa: "دفتر کوته‌سنگی", nameEn: "Kote Sangi office" },
      { cityFa: "کابل", cityEn: "Kabul", nameFa: "دفتر کمپنی", nameEn: "Kampani office" },
      { cityFa: "غزنی", cityEn: "Ghazni", nameFa: "دفتر غزنی", nameEn: "Ghazni office" },
      { cityFa: "قلات", cityEn: "Qalat", nameFa: "دفتر قلات", nameEn: "Qalat office" },
      { cityFa: "کندهار", cityEn: "Kandahar", nameFa: "دفتر اصلی", nameEn: "Main office" },
      { cityFa: "کندهار", cityEn: "Kandahar", nameFa: "دفتر باغ پل", nameEn: "Bagh-e Pol office" },
      { cityFa: "هلمند", cityEn: "Helmand", nameFa: "دفتر هلمند", nameEn: "Helmand office" },
      { cityFa: "نیمروز", cityEn: "Nimroz", nameFa: "دفتر نیمروز", nameEn: "Nimroz office" },
      { cityFa: "فراه", cityEn: "Farah", nameFa: "دفتر فراه", nameEn: "Farah office" },
      { cityFa: "هرات", cityEn: "Herat", nameFa: "دفتر هرات", nameEn: "Herat office" },
    ],
    common: {
      back: "Back",
      continue: "Continue",
      currency: "AFN",
      total: "Total",
      seat: "seat",
      seats: "seats",
      passenger: "passenger",
      passengers: "passengers",
      to: "to",
      night: "overnight",
      day: "daytime",
      close: "Close",
      optional: "optional",
      placeholder: "PLACEHOLDER",
    },
    search: {
      resultsTitle: "Available trips",
      resultsSub: "trips found for this route",
      editSearch: "Edit search",
      departure: "Departure",
      arrival: "Arrival",
      duration: "Duration",
      seatsLeft: "seats left",
      full: "Full",
      selectSeats: "Select seats",
      sortTitle: "Sort by",
      sortEarliest: "Earliest",
      sortCheapest: "Cheapest",
      sortFastest: "Fastest",
      filtersTitle: "Filters",
      timeOfDay: "Departure time",
      morning: "Morning",
      afternoon: "Afternoon",
      evening: "Evening",
      amenities: "Amenities",
      busType: "Bus type",
      priceRange: "Price range",
      upTo: "Up to",
      noResults: "No trips found for this route.",
      hours: "h",
      flexibleDeparture: "Departs when full",
    },
    amenities: {
      ac: "Air conditioning",
      wifi: "Wi-Fi",
      charging: "Charging",
      refreshment: "Refreshments",
      reclining: "Reclining seats",
    },
    busTypes: {
      vip: "VIP 1+2",
      standard: "Standard 2+2",
    },
    seats: {
      title: "Choose your seats",
      coach: "Coach",
      driver: "Driver",
      door: "Door",
      legendAvailable: "Available",
      legendBooked: "Booked",
      legendSelected: "Selected",
      selectedTitle: "Selected seats",
      none: "No seats selected yet.",
      max: "Maximum number of seats selected.",
      pricePerSeat: "Price per seat",
      continueToCheckout: "Continue to checkout",
      seatLabel: "Seat number",
      statusAvailable: "available",
      statusBooked: "booked",
      statusSelected: "selected",
      layoutNote: "This service's layout",
      holdError: "Unfortunately one of your selected seats was just booked by another passenger. The seat map has been refreshed; please choose again.",
      holdErrorGeneric: "Something went wrong locking your seats. Please try again.",
      holding: "Checking...",
    },
    checkout: {
      title: "Complete your booking",
      passengerDetails: "Passenger details",
      passengerNo: "Passenger",
      fullName: "Full name",
      fullNamePh: "First and last name",
      nationalId: "National ID",
      nationalIdPh: "e-Tazkira number",
      gender: "Gender",
      male: "Male",
      female: "Female",
      contactTitle: "Contact information",
      phone: "Phone number",
      email: "Email (optional)",
      luggageTitle: "Extra luggage (optional)",
      luggageOption: "I'd like to register extra luggage",
      luggageNote:
        "[PLACEHOLDER — needs real pricing from the company] The exact extra-luggage fee is not yet confirmed; if selected, this is finalized with the operator when boarding.",
      paymentTitle: "Payment method",
      payCard: "Bank card (online)",
      payCardDesc: "Secure online payment by bank card via the HesabPay gateway.",
      payMobile: "Mobile money (online)",
      payMobileDesc: "Online payment via your mobile wallet, through the same HesabPay gateway.",
      payOffice: "Offline payment",
      payOfficeDesc: "Pay in cash at the office or when boarding; the booking stays \"pending confirmation\" until paid.",
      summaryTitle: "Trip summary",
      route: "Route",
      date: "Date",
      seatsLabel: "Seats",
      subtotal: "Tickets subtotal",
      serviceFee: "Service fee",
      grandTotal: "Amount due",
      pay: "Pay and issue tickets",
      required: "This field is required",
      termsLabel: "I accept the booking terms & conditions and the privacy policy.",
      termsLink: "View terms & conditions",
      privacyLink: "Privacy policy",
      termsRequired: "You must accept the terms to continue",
      couponTitle: "Coupon code (optional)",
      couponPh: "e.g. SHABRAW10",
      couponInvalid: "This coupon code is invalid or expired.",
      couponMinSeats: "This coupon requires booking at least {n} seat(s).",
      couponMinAmount: "This coupon requires a minimum order of {n} {currency}.",
      couponRouteNotEligible: "This coupon isn't valid for this route.",
      couponRegisteredOnly: "This coupon is only for registered customers; please sign in, or continue your booking without this code.",
      couponTierTooLow: "This coupon requires a higher loyalty tier.",
      couponFirstTripOnly: "This coupon is only for new customers' first trip.",
      couponPerCustomerLimit: "You've already used this coupon the maximum number of times.",
      couponAppliedAtConfirm: "The coupon discount is calculated once you confirm the booking, and shown on the confirmation page.",
      onlinePendingNote:
        "The online payment gateway (HesabPay) isn't fully connected yet; your booking will be created as \"pending confirmation\" and an operator will contact you shortly.",
      holdExpired: "Your seat hold has expired. Please go back to seat selection and choose again.",
      genericError: "We couldn't complete your booking. Please try again.",
      submitting: "Submitting booking...",
    },
    confirm: {
      title: "Your booking is confirmed",
      subtitle: "Have a pleasant journey. Your ticket details were sent to your phone number.",
      ref: "Booking reference",
      route: "Route",
      date: "Travel date",
      departure: "Departure time",
      seats: "Seats",
      passenger: "Passenger",
      amount: "Amount paid",
      boarding: "Please arrive at the terminal 30 minutes before departure and keep your booking reference handy.",
      download: "Download ticket",
      addToCalendar: "Add to calendar",
      home: "Back to home",
      qrHint: "Show this QR code when boarding",
      supportTitle: "Emergency travel support",
      supportNote:
        "[PLACEHOLDER — needs the company's real support number] If anything comes up before or during your trip, reach us through the contact page or your nearest office.",
      contactOffices: "View support offices",
      statusPendingOffline: "Pending payment confirmation — please pay when boarding or at an office.",
      statusPendingOnline: "Pending online payment confirmation — an operator will contact you shortly.",
    },
    about: {
      kicker: "About us",
      title: "A network connecting Kabul to Herat",
      intro:
        "Shabraw carries passengers along Afghanistan's southwest corridor — from Kabul to Herat, stopping in Ghazni, Qalat, Kandahar, Helmand, Nimroz, and Farah — with a fleet of large intercity coaches and a regular timetable.",
      stats: [
        { label: "Years in operation" },
        { label: "Cities served" },
        { label: "Coaches in the fleet" },
        { label: "Daily departures" },
      ],
      statsPlaceholderNote: "[PLACEHOLDER] These figures will be completed once real numbers are provided by the company.",
      storyTitle: "Our story",
      storyBody:
        "Our network covers a single corridor: Kabul, Ghazni, Qalat, Kandahar, Helmand, Nimroz, Farah, and Herat. Passengers can book between any two points on this route.",
      safetyTitle: "Our commitment to safety and quality",
      safetyBody:
        "Every trip runs on model \"580\" coaches (Mercedes-Benz Travego/O580), built for long intercity journeys. Pre-departure technical checks and schedule reliability are core priorities.",
      fleetPhotoNote: "[PLACEHOLDER — needs real photos of the company's offices and coaches]",
      busTypesTitle: "Two service levels, one standard of comfort",
      busTypesIntro: "Depending on your route and budget, choose between two levels of service.",
      vipDesc:
        "A 1+2 layout with extra legroom and lower capacity than standard service, air conditioning, Wi-Fi, charging, and refreshments along the way.",
      standardDesc:
        "A 2+2 layout with comfortable seats and air conditioning — an affordable choice for everyday travel.",
      ctaTitle: "Ready for your next trip?",
      ctaButton: "Search trips",
    },
    contact: {
      kicker: "Contact us",
      title: "Get in touch",
      subtitle:
        "For in-person booking, tracking a ticket, or any other question, visit your nearest office or send us a message.",
      officesTitle: "Sales offices",
      hours: "Daily, 6 AM to 10 PM",
      hoursNote: "[PLACEHOLDER — each office's exact hours still need confirmation]",
      mapTitle: "Corridor map",
      mapNote: "This map shows the geographic order of cities we cover; a precise office map will be added once addresses are confirmed.",
      formTitle: "Send a message",
      formName: "Your name",
      formNamePh: "First and last name",
      formPhone: "Phone number",
      formMessage: "Message",
      formMessagePh: "Write your question or request…",
      formSubmit: "Send message",
      required: "This field is required",
      sent: "Your message has been sent. We'll contact you soon.",
    },
    faq: {
      kicker: "FAQ",
      title: "Frequently asked questions",
      subtitle: "Find answers to common questions about booking, payment, luggage, and our cancellation policy.",
      groups: [
        {
          title: "Booking",
          items: [
            {
              q: "Do I need an account to book?",
              a: "No. Bookings only require a phone number, and you'll receive a booking reference. Creating an account is optional.",
            },
            {
              q: "What's the maximum number of seats per booking?",
              a: "Up to six seats per booking. For larger groups, please contact your nearest office.",
            },
            {
              q: "Can I book a ticket for someone else?",
              a: "Yes. Just enter that person's name and national ID in the passenger form; the booking phone number can be your own.",
            },
          ],
        },
        {
          title: "Payment",
          items: [
            {
              q: "What payment methods are supported?",
              a: "Online payment by bank card or mobile money via the HesabPay gateway, as well as cash payment at the office or when boarding.",
            },
            {
              q: "If I choose offline payment, when is my ticket finalized?",
              a: "Offline-payment bookings are recorded as \"pending confirmation\" and finalized once the operator confirms payment.",
            },
            {
              q: "Is online payment secure?",
              a: "Online payment runs through the HesabPay gateway, and your card details are never stored directly on our servers.",
            },
          ],
        },
        {
          title: "Cancellation and refund policy",
          items: [
            {
              q: "Can I cancel my booking?",
              a: "Yes. Cancellations made 24 hours or more before departure are free and fully refunded. Cancellations within 24 hours of departure incur a 20% fee.",
            },
            {
              q: "How long does a refund take?",
              a: "Card and mobile-money refunds are usually processed within three to seven business days. Cash payments are refunded at the office where you booked.",
            },
            {
              q: "Can I change my travel date?",
              a: "Yes, date changes are free up to 12 hours before departure, subject to seat availability on the new trip.",
            },
          ],
        },
        {
          title: "Luggage and baggage",
          items: [
            {
              q: "How much luggage can I bring?",
              a: "[PLACEHOLDER — needs exact allowance from the company] The exact weight and size allowance is not yet confirmed; contact your office before travel for specifics.",
            },
            {
              q: "What if I have extra luggage?",
              a: "[PLACEHOLDER — needs real pricing from the company] You can select \"extra luggage\" on the booking form; the exact fee and terms are arranged with the operator when boarding.",
            },
            {
              q: "What items are prohibited?",
              a: "Flammable materials, weapons and ammunition, and hazardous or illegal goods are not permitted. Full details are on the luggage policy page.",
            },
          ],
        },
        {
          title: "On the day of travel",
          items: [
            {
              q: "How early should I arrive at the terminal?",
              a: "Please arrive at least 30 minutes before departure and keep your booking reference handy.",
            },
            {
              q: "Do children need a separate ticket?",
              a: "Children under 2 travelling on a parent's lap travel free; a full ticket is required for children over 2.",
            },
            {
              q: "Are there stops along the way?",
              a: "Yes, depending on the route length, one or more short stops are scheduled for rest and prayer.",
            },
          ],
        },
      ],
    },
    luggagePage: {
      kicker: "Travel policy",
      title: "Luggage and baggage policy",
      subtitle: "Check the allowed baggage, extra luggage terms, and prohibited items before you travel.",
      allowanceTitle: "Baggage allowance per passenger",
      allowanceBody: "[PLACEHOLDER — needs exact allowance from the company] The exact weight and size allowance (hold luggage and carry-on) has not yet been provided by the company.",
      extraTitle: "Extra luggage",
      extraBody:
        "[PLACEHOLDER — needs real pricing from the company] If you have luggage beyond the allowance, select \"extra luggage\" when booking. The exact fee and terms are arranged with the operator when boarding.",
      prohibitedTitle: "Prohibited items",
      prohibitedItems: [
        "Flammable or explosive materials",
        "Weapons, ammunition, and unpackaged sharp objects",
        "Hazardous or toxic chemicals",
        "Goods illegal under the laws of the Islamic Republic of Afghanistan",
      ],
      note: "This policy is a general framework and must be completed with the company's exact figures and terms before launch.",
    },
    terms: {
      kicker: "Terms & conditions",
      title: "Terms of use",
      updatedLabel: "Last updated",
      updatedValue: "[PLACEHOLDER]",
      notice:
        "[PLACEHOLDER — this is an initial framework and must be reviewed and completed by the company's legal/management team before launch.]",
      sections: [
        {
          heading: "1. Acceptance of terms",
          body: "By booking a ticket through this website, you accept the terms and conditions below.",
        },
        {
          heading: "2. Booking and ticket issuance",
          body: "Bookings are made by entering passenger details and a valid phone number. The digital ticket is issued once payment is confirmed.",
        },
        {
          heading: "3. Cancellation and refunds",
          body: "The cancellation and refund policy is described on the FAQ page and forms part of these terms.",
        },
        {
          heading: "4. Responsibility for luggage",
          body: "The passenger is responsible for accurately declaring luggage and following the luggage policy.",
        },
        {
          heading: "5. Schedule changes",
          body: "The company reserves the right to change or delay the travel schedule when necessary (weather, technical, or security conditions).",
        },
      ],
    },
    privacy: {
      kicker: "Privacy",
      title: "Privacy policy",
      updatedLabel: "Last updated",
      updatedValue: "[PLACEHOLDER]",
      notice:
        "[PLACEHOLDER — this is an initial framework and must be reviewed and completed by the company's legal/management team before launch.]",
      sections: [
        {
          heading: "1. Information we collect",
          body: "Passenger name, national ID number, phone number, and email if provided — collected solely to issue and track your ticket.",
        },
        {
          heading: "2. How we use it",
          body: "Your information is used only to manage your booking, send travel notifications, and provide customer support.",
        },
        {
          heading: "3. Sharing information",
          body: "Passenger information is not shared with third parties, aside from the payment gateway (to process transactions) and where required by law.",
        },
        {
          heading: "4. Data retention",
          body: "Booking information is retained for as long as needed for follow-up and internal reporting.",
        },
      ],
    },
    admin: {
      title: "Admin panel",
      subtitle: "Network operations overview",
      nav: {
        dashboard: "Dashboard",
        trips: "Trips",
        bookings: "Bookings",
        buses: "Fleet",
        routes: "Routes",
        cities: "Cities",
        drivers: "Drivers",
        reports: "Reports",
        loyalty: "Loyalty",
        content: "Site content",
        payments: "Payments",
        admins: "Admin management",
      },
      exit: "Exit panel",
      stats: {
        bookings: "Today's bookings",
        revenue: "Today's revenue",
        occupancy: "Avg. occupancy",
        trips: "Active trips",
      },
      vsYesterday: "vs. yesterday",
      recentTitle: "Recent bookings",
      recentSub: "Latest issued tickets",
      searchPh: "Search reference or passenger name…",
      upcomingTitle: "Upcoming trips",
      cols: {
        ref: "Ref",
        passenger: "Passenger",
        route: "Route",
        date: "Date",
        seats: "Seats",
        amount: "Amount",
        status: "Status",
        time: "Departs",
        occupancy: "Occupancy",
        bus: "Bus",
        seatNumbers: "Seats booked",
        registeredAt: "Booked on",
      },
      status: {
        confirmed: "Confirmed",
        pending: "Pending",
        cancelled: "Cancelled",
        completed: "Completed",
        refunded: "Refunded",
      },
      allStatuses: "All statuses",
      manage: {
        addNew: "Add new",
        edit: "Edit",
        delete: "Delete",
        save: "Save",
        cancel: "Cancel",
        confirmDeleteTitle: "Delete this?",
        genericError: "Something went wrong. Please try again.",
        active: "Active",
        inactive: "Inactive",
        optional: "optional",
      },
      routes: {
        title: "Route management",
        subtitle: "Active routes between corridor cities",
        addRoute: "New route",
        editRoute: "Edit route",
        origin: "Origin city",
        destination: "Destination city",
        distanceKm: "Distance (km)",
        durationMinutes: "Typical duration (minutes)",
        isActive: "This route is active",
        empty: "No routes yet.",
        sameCityError: "Origin and destination can't be the same.",
        duplicateError: "This route (same origin and destination) already exists.",
        deleteConfirmBody: "This route will be permanently deleted. This can't be undone.",
        deleteInUseError: "This route has trips scheduled and can't be deleted; deactivate it instead.",
      },
      cities: {
        title: "Cities & provinces",
        subtitleCount: "{active} active of {total} provinces",
        searchPlaceholder: "Search city name…",
        nameFa: "Name (Dari)",
        nameEn: "Name (English)",
        empty: "No cities found.",
      },
      siteContent: {
        title: "Site content",
        subtitle:
          "Company contact, \"about us\" stats, the real fleet photo, and offices — these appear on public pages (footer, homepage, about, contact) in place of the current placeholders.",
        settingsSectionTitle: "Global settings",
        companyPhone: "Company phone number",
        companyPhonePlaceholder: "e.g. +93 70 000 0000",
        companyEmail: "Company email",
        companyEmailPlaceholder: "info@example.com",
        statsSectionTitle: "\"About us\" stats",
        statsSectionSubtitle:
          "Shown on the homepage and the about page. Leave a field empty to keep its current placeholder (\"—\").",
        yearsActive: "Years active",
        citiesCovered: "Cities covered",
        busesInFleet: "Buses in fleet",
        dailyTrips: "Daily trips",
        fleetPhotoSectionTitle: "Real fleet photo",
        fleetPhotoSectionSubtitle:
          "Shown in the \"Safety & quality commitment\" section of the about page, replacing the placeholder note.",
        fleetPhotoUpload: "Upload photo",
        fleetPhotoReplace: "Replace photo",
        fleetPhotoRemove: "Remove photo",
        fleetPhotoUploading: "Uploading…",
        fleetPhotoNone: "No photo uploaded yet.",
        fleetPhotoTooLarge: "Photo must be under 5 MB.",
        fleetPhotoInvalidType: "Only PNG, JPEG, or WebP is allowed.",
        saveSettings: "Save settings",
        settingsSaved: "Settings saved.",
        officesSectionTitle: "Offices",
        officesSectionSubtitle: "Address, phone, and hours for each office — independent of the others.",
        addOffice: "New office",
        editOffice: "Edit office",
        officeCity: "City",
        officeNameFa: "Office name (Dari)",
        officeNameEn: "Office name (English)",
        officeAddressFa: "Address (Dari)",
        officeAddressEn: "Address (English)",
        officePhone: "Office phone number",
        officeHoursFa: "Working hours (Dari)",
        officeHoursEn: "Working hours (English)",
        officeIsActive: "Show this office on the site",
        officeEmpty: "No offices yet.",
        officeDeleteConfirmBody: "This office will be permanently deleted. This can't be undone.",
        colOffice: "Office",
        colCity: "City",
        colPhone: "Phone",
      },
      responsiveImages: {
        title: "Responsive images",
        subtitle:
          "Each background section has 4 separate versions for mobile/tablet/desktop/wide — upload and crop each one independently. Until something is uploaded, the site's current photo stays in place.",
        upload: "Upload",
        replace: "Replace",
        reset: "Reset to default",
        resetConfirmTitle: "Reset to the default photo?",
        resetConfirmBody: "This slot will show the site's current photo again; the uploaded file isn't deleted, it just stops being used.",
        defaultBadge: "Site default",
        customBadge: "Admin upload",
        cropTitle: "Crop photo",
        cropHint: "Drag to reposition and use the slider to zoom — the frame matches exactly the ratio shown on the site.",
        zoomLabel: "Zoom",
        apply: "Apply & upload",
        uploading: "Uploading…",
        invalidType: "Only PNG, JPEG, or WebP is allowed.",
        tooLarge: "Photo must be under 15 MB.",
      },
      buses: {
        title: "Fleet management",
        subtitle: "Company buses",
        addBus: "New bus",
        editBus: "Edit bus",
        code: "Bus code",
        plateNumber: "Plate number",
        totalSeats: "Capacity (seats)",
        status: "Status",
        empty: "No buses yet.",
        duplicateCodeError: "This bus code is already in use.",
        deleteConfirmBody: "This bus will be permanently deleted. Past trips on this bus stay unchanged.",
      },
      busStatus: {
        active: "Active",
        maintenance: "Maintenance",
        retired: "Retired",
      },
      drivers: {
        title: "Driver management",
        subtitle: "Company drivers",
        addDriver: "New driver",
        editDriver: "Edit driver",
        fullName: "Full name",
        phone: "Phone number",
        licenseNumber: "License number",
        status: "Status",
        empty: "No drivers yet.",
        duplicatePhoneError: "This phone number is already registered to another driver.",
        deleteConfirmBody: "This driver will be permanently deleted.",
      },
      driverStatus: {
        active: "Active",
        inactive: "Inactive",
      },
      scheduler: {
        title: "Trip scheduling",
        subtitle: "Define new trips and manage existing ones",
        addTrip: "New trip",
        editTrip: "Edit trip",
        route: "Route",
        selectRoute: "Select a route",
        bus: "Bus",
        noBusYet: "No bus yet (assign later)",
        driver: "Driver",
        noDriverYet: "No driver yet (assign later)",
        serviceDate: "Travel date",
        scheduleType: "Departure type",
        fixedTime: "Fixed time",
        fillAndGo: "Fill and go",
        departureTime: "Departure time",
        pricePerSeat: "Price per seat",
        seatLayout: "Seat layout",
        totalSeats: "Total seats",
        status: "Trip status",
        empty: "No trips defined yet.",
        noRoutesWarning: "You need at least one active route first (see the Routes section).",
        deleteConfirmBody: "This trip will be permanently deleted.",
        deleteHasBookingsError: "This trip has bookings and can't be deleted; change its status to \"Cancelled\" instead.",
        seatsLockedNote: "Once a trip is created, its seat count and layout can no longer be changed.",
        filters: {
          all: "All",
          today: "Today",
          scheduled: "Upcoming",
          departed: "Departed",
          completed: "Arrived",
          cancelled: "Cancelled",
        },
        capacity: "Capacity",
        delayed: "Possibly delayed",
        markDeparted: "Mark departed",
        markArrived: "Mark arrived",
        cancelTrip: "Cancel trip",
        cancelTripConfirmTitle: "Cancel this trip?",
        cancelTripConfirmBodyNoBookings: "This trip has no active bookings and can be cancelled safely.",
        cancelTripConfirmBodyWithBookings: "This trip has {count} active booking(s). Cancelling the trip does not cancel the bookings themselves — passengers must be contacted separately.",
        departedAt: "Departure time",
        arrivedAt: "Arrival time",
      },
      tripStatus: {
        scheduled: "Scheduled",
        boarding: "Boarding",
        departed: "Departed",
        completed: "Completed",
        cancelled: "Cancelled",
      },
      paymentStatus: {
        pending: "Pending",
        confirmed: "Confirmed",
        failed: "Failed",
        refunded: "Refunded",
      },
      bookingsPanel: {
        title: "Booking management",
        subtitle: "All bookings placed on real trips",
        empty: "No bookings yet.",
        contact: "Passenger/Contact",
        paymentMethod: "Payment method",
        paymentStatus: "Payment status",
        online: "Online",
        offline: "Offline",
        guest: "Guest",
        confirmPayment: "Confirm payment",
        confirmPaymentConfirmTitle: "Confirm this payment?",
        confirmPaymentConfirmBody: "This offline payment will be marked as received and the booking confirmed.",
        cancelBooking: "Cancel booking",
        cancelBookingConfirmTitle: "Cancel this booking?",
        cancelBookingConfirmBody: "This booking will be cancelled and its seats released back to available.",
        refundPayment: "Refund",
        refundPaymentConfirmTitle: "Refund this payment?",
        refundPaymentConfirmBody: "This booking's seats will be released back to available and the booking marked as refunded. This can't be undone.",
        refundAmountLabel: "Refund amount",
        refundFullAmountHint: "Full payment amount",
        refundReasonLabel: "Refund reason",
        refundInvalidAmount: "Amount must be between zero and the full payment amount.",
      },
      paymentsPanel: {
        title: "Payment history",
        subtitle: "Every payment status change, from booking creation through confirmation and refund",
        empty: "No events yet.",
        colTransition: "Transition",
        colAmount: "Amount",
        colActor: "Actor",
        colNote: "Note",
        newPayment: "Initial",
        actorAdmin: "Admin",
        actorCustomer: "Passenger",
        actorSystem: "System",
      },
      reportsPanel: {
        title: "Statistical reports",
        subtitle: "Revenue, passenger count, and seat occupancy, broken down by route and travel date",
        from: "From date",
        to: "To date",
        last7: "Last 7 days",
        last30: "Last 30 days",
        last90: "Last 90 days",
        allTime: "All time",
        routeFilter: "Route",
        allRoutes: "All routes",
        totalRevenue: "Total revenue",
        totalPassengers: "Total passengers",
        avgOccupancy: "Avg. occupancy",
        totalTrips: "Total trips",
        byRouteTitle: "By route",
        byDateTitle: "By date",
        exportCsv: "Export CSV",
        colRoute: "Route",
        colDate: "Date",
        colTrips: "Trips",
        colPassengers: "Passengers",
        colRevenue: "Revenue",
        colOccupancy: "Occupancy",
        empty: "No trips found in this date range.",
        invalidRange: "\"From\" date must be on or before the \"To\" date.",
      },
      loyaltyPanel: {
        title: "Loyalty program management",
        subtitle: "Tier thresholds and discounts, referral reward, and seasonal discount codes",
        tiersTitle: "Membership tiers",
        tiersSubtitle: "Trip threshold and discount percent per tier — changeable without a code change",
        colTier: "Tier",
        colThreshold: "Threshold (completed trips)",
        colDiscount: "Discount %",
        colTierStatus: "Status",
        editTier: "Edit tier",
        thresholdLabel: "Minimum completed trips",
        discountLabel: "Discount percent",
        tierActiveLabel: "This tier is active",
        thresholdRangeError: "This value must be zero or greater.",
        discountRangeError: "Discount percent must be between 0 and 100.",
        nonIncreasingWarning: "Warning: this threshold isn't consistent with the tier below or above it; the natural tier order may break.",
        referralTitle: "Referral reward",
        referralSubtitle:
          "This amount is credited to both parties' wallets (referrer and referred) after the referred customer completes their first trip.",
        referralAmountLabel: "Reward amount (per party)",
        referralSaved: "Saved.",
        couponsTitle: "Discount codes",
        couponsSubtitle: "Seasonal campaigns, independent of tier discounts",
        addCoupon: "New coupon",
        editCoupon: "Edit coupon",
        code: "Coupon code",
        codeHelper: "Automatically saved in uppercase.",
        discountType: "Discount type",
        discountTypePercent: "Percent",
        discountTypeFixed: "Fixed amount",
        discountValue: "Discount value",
        stackableWithTier: "Stackable with tier discount",
        usageLimit: "Usage limit",
        usageLimitHelper: "Leave empty for unlimited.",
        usedCount: "Used",
        unlimited: "Unlimited",
        validFrom: "Valid from",
        validTo: "Valid to",
        noDateLimit: "—",
        couponActive: "This coupon is active",
        yes: "Yes",
        no: "No",
        empty: "No coupons yet.",
        duplicateCodeError: "This coupon code is already in use.",
        invalidDiscountValue: "The discount value must be greater than zero.",
        invalidPercentValue: "Percent discount must be between 1 and 100.",
        invalidDateRangeError: "The \"valid to\" date must be on or after the \"valid from\" date.",
        deleteConfirmBody: "This coupon will be permanently deleted.",
        deleteUsedError:
          "This coupon has already been used and deleting it would destroy the related booking history; deactivate it instead.",
        advancedRulesTitle: "Advanced rules (optional — empty/unchecked means no restriction)",
        minLoyaltyTier: "Minimum loyalty tier",
        noTierRestriction: "No tier restriction",
        perCustomerLimit: "Usage limit per customer",
        minSeats: "Minimum seats",
        minAmount: "Minimum order amount",
        applicableRoutes: "Restrict to specific routes",
        applicableRoutesHelper: "Leave all unchecked to apply to every route.",
        noRoutesYet: "No routes yet.",
        firstTripOnly: "First-time customers only",
        registeredOnly: "Registered customers only (not guest checkout)",
      },
      admins: {
        title: "Admin management",
        subtitle: "Set each admin's role and access — only super admins see this tab.",
        addAdmin: "Invite new admin",
        colName: "Name",
        colEmail: "Email",
        colRole: "Role",
        colSections: "Access",
        colStatus: "Status",
        colLastSignIn: "Last sign-in",
        neverSignedIn: "Not signed in yet",
        youLabel: "(you)",
        roleSuperAdmin: "Super admin",
        roleLimitedAdmin: "Limited admin",
        allSectionsLabel: "All sections",
        noSectionsLabel: "No access",
        empty: "No admins yet.",
        inviteTitle: "Invite new admin",
        editTitle: "Edit admin",
        fullNameLabel: "Full name",
        emailLabel: "Email",
        emailHelper: "An invite email to set a password will be sent to this address.",
        roleLabel: "Role",
        sectionsLabel: "Section access",
        sectionsHelper: "Super admins get full access to every section — no selection needed.",
        inviteSubmit: "Send invite",
        inviteSending: "Sending...",
        selfRoleLocked: "You can't change your own role — this prevents locking yourself out.",
        selfDeactivateLocked: "You can't deactivate your own account — this prevents locking yourself out.",
        errorMissingFullName: "Enter a full name.",
        errorInvalidEmail: "This email isn't valid.",
        errorEmailTaken: "This email is already registered (as an admin or a customer).",
        errorEmailIsCustomer: "This email belongs to a customer account — use a separate email for the admin.",
        errorInviteFailed: "Sending the invite failed. Please try again.",
        errorSelfRoleChange: "You can't change your own account's role — ask another super admin.",
        errorSelfDeactivate: "You can't deactivate your own account — ask another super admin.",
        managerBadge: "Manager",
        managerScopeNotice:
          "You're a \"Manager\": you can create/edit/deactivate limited admins, but you can't access super admin accounts, and you can't make anyone (including yourself) a super admin or a manager.",
        canManageAdminsLabel: "This account can manage admins (Manager)",
        canManageAdminsHelper: "Only a super admin can grant or revoke this. Someone with this flag can create/edit other limited admins, but can't create a super admin or grant this flag to anyone.",
        roleFixedLimitedNote: "You can only create limited admins.",
        errorManagerCannotTouchSuperAdmin: "You don't have access to super admin accounts.",
        errorManagerCannotGrantSuperAdmin: "Only a super admin can make someone a super admin.",
        errorManagerCannotGrantManageFlag: "Only a super admin can grant the \"Manager\" permission.",
        sections: {
          routes: "Routes & Cities",
          fleet: "Fleet (buses & drivers)",
          trips: "Trips",
          bookings: "Bookings (also gates dashboard & reports)",
          payments: "Payments",
          loyalty: "Loyalty program",
          customers: "Customers",
          content: "Site content (contact, offices, stats)",
        },
        auditTitle: "Access change history",
        auditToggleShow: "Show history",
        auditToggleHide: "Hide history",
        auditEmpty: "No changes recorded yet.",
        auditActor: "by",
        auditChange: {
          created: "created",
          role_changed: "role changed",
          sections_changed: "access changed",
          activated: "activated",
          deactivated: "deactivated",
          renamed: "renamed",
          manage_flag_changed: "manager permission changed",
        },
      },
    },
    account: {
      loginTitle: "Sign in to your account",
      loginSubtitle: "Sign in with your email and password to access your booking history, wallet, and referral code.",
      signupTitle: "Create a passenger account",
      signupSubtitle: "Signing up enrolls you in the loyalty club: tier discounts, a wallet, and a friend referral code — entirely optional, guest checkout is still available.",
      emailLabel: "Email",
      passwordLabel: "Password",
      fullNameLabel: "Full name (optional)",
      phoneLabel: "Phone number",
      phonePlaceholder: "07xxxxxxxx",
      referralCodeLabel: "Referral code (optional)",
      loginSubmit: "Sign in",
      forgotPasswordLink: "Forgot your password?",
      signupSubmit: "Sign up",
      noAccountYet: "Don't have an account yet?",
      goSignup: "Sign up",
      haveAccount: "Already have an account?",
      goLogin: "Sign in",
      confirmEmailNotice: "A confirmation email has been sent. After confirming, sign in again to finish setting up your loyalty profile.",
      completeProfileTitle: "Complete your profile",
      completeProfileSubtitle: "One step left — enter your phone number to activate the loyalty club.",
      completeProfileSubmit: "Finish signup",
      required: "This field is required",
      errorInvalidCredentials: "Incorrect email or password.",
      errorEmailInUse: "This email is already registered.",
      errorWeakPassword: "Password must be at least 6 characters.",
      errorPhoneTaken: "This phone number is already registered to another account.",
      errorInvalidReferral: "The referral code you entered isn't valid.",
      errorGeneric: "Something went wrong. Please try again.",
      backToLogin: "Back to sign in",
      forgotPasswordTitle: "Reset your password",
      forgotPasswordSubtitle: "Enter your account email — if it's registered, we'll send a password reset link to it.",
      forgotPasswordSubmit: "Send reset link",
      forgotPasswordSuccessTitle: "Email sent",
      forgotPasswordSuccessBody: "If that email is registered, you'll receive a password reset link within a few minutes. Check your spam folder too.",
      resetPasswordTitle: "Set a new password",
      resetPasswordSubtitle: "Choose a new password for your account.",
      newPasswordLabel: "New password",
      confirmPasswordLabel: "Confirm new password",
      resetPasswordSubmit: "Save new password",
      resetPasswordSuccessTitle: "Password updated",
      resetPasswordSuccessBody: "Your password was updated successfully.",
      goToDashboard: "Go to my account",
      passwordMismatch: "Password and confirmation don't match.",
      resetLinkInvalidTitle: "Invalid or expired link",
      resetLinkInvalidBody: "This password reset link is no longer valid, or it's already been used.",
      requestNewLink: "Request a new link",
      dashboardTitle: "My account",
      logout: "Sign out",
      tierTitle: "Loyalty tier",
      tripsCompletedLabel: "Completed trips",
      tierDiscountLabel: "Your permanent discount",
      tierMaxReached: "You're at the top loyalty tier.",
      tripsToNextTierPrefix: "Just",
      tripsToNextTierSuffix: "more trip(s) to reach",
      walletTitle: "Wallet",
      walletBalanceLabel: "Current balance",
      walletEmpty: "No wallet transactions yet.",
      walletTxType: {
        cashback: "Cashback",
        referral_bonus: "Referral bonus",
        redeemed: "Used on a booking",
        manual_adjustment: "Manual adjustment",
      },
      referralTitle: "Friend referral code",
      referralSubtitle: "Share this code with friends; once their first trip is completed, you both get rewarded.",
      referralCopy: "Copy code",
      referralCopied: "Copied!",
      referralCompletedCountLabel: "Successful referrals",
      bookingHistoryTitle: "Booking history",
      bookingHistoryEmpty: "No bookings under this account yet.",
      startBooking: "Search a trip",
      viewBookingLink: "View details",
    },
  },
}

/**
 * The real, confirmed operating corridor (from the company's approved office
 * list): a single southwest line from Kabul to Herat. Listed in geographic
 * order along that corridor. Do NOT add cities here without confirmation —
 * see claude_project-master-prompt.md "no-fabrication" rule.
 */
export const cities: { fa: string; en: string }[] = [
  { fa: "کابل", en: "Kabul" },
  { fa: "غزنی", en: "Ghazni" },
  { fa: "قلات", en: "Qalat" },
  { fa: "کندهار", en: "Kandahar" },
  { fa: "هلمند", en: "Helmand" },
  { fa: "نیمروز", en: "Nimroz" },
  { fa: "فراه", en: "Farah" },
  { fa: "هرات", en: "Herat" },
]

export type Route = {
  from: { fa: string; en: string }
  to: { fa: string; en: string }
  image: string
  hours: number
  price: number
  trips: number
  accent: string
}

/**
 * Sample routes for the homepage teaser strip. Prices/trip-counts are demo
 * values for the interactive booking flow (kept functional per the parent
 * project's UI/UX phase) — NOT the confirmed real pricing. The static
 * "Routes & prices" page shows explicit [PLACEHOLDER] pricing instead; see
 * routesPage.priceNote.
 */
export const popularRoutes: Route[] = [
  {
    from: { fa: "کابل", en: "Kabul" },
    to: { fa: "هرات", en: "Herat" },
    image: "/images/city-herat.png",
    hours: 13,
    price: 1800,
    trips: 6,
    accent: "var(--chart-1)",
  },
  {
    from: { fa: "کابل", en: "Kabul" },
    to: { fa: "کندهار", en: "Kandahar" },
    image: "/images/city-kandahar.png",
    hours: 6,
    price: 950,
    trips: 8,
    accent: "var(--chart-2)",
  },
  {
    from: { fa: "کندهار", en: "Kandahar" },
    to: { fa: "هرات", en: "Herat" },
    image: "/images/city-herat.png",
    hours: 7,
    price: 1000,
    trips: 5,
    accent: "var(--chart-4)",
  },
  {
    from: { fa: "کابل", en: "Kabul" },
    to: { fa: "غزنی", en: "Ghazni" },
    // [PLACEHOLDER] فاز ۴.۸ (ردیف #۴ — متوسط): عکس واقعی غزنی هنوز وجود
    // ندارد؛ فعلاً عمداً از عکس کابل (مبدأ) به‌عنوان placeholder موقت
    // استفاده می‌شود تا کارت خالی نماند. باید با پرامپت اصلی v0.dev
    // (طبق بخش ۸.۲ سند مادر) جایگزین شود — نیازمند تأیید/ارسال پرامپت از
    // Zakir، نه ساخت حدسی.
    image: "/images/city-kabul.png",
    hours: 2,
    price: 300,
    trips: 10,
    accent: "var(--chart-3)",
  },
]

const faDigitMap: Record<string, string> = {
  "0": "۰",
  "1": "۱",
  "2": "۲",
  "3": "۳",
  "4": "۴",
  "5": "۵",
  "6": "۶",
  "7": "۷",
  "8": "۸",
  "9": "۹",
}

export function localizeNumber(value: number | string, lang: Lang): string {
  const str = typeof value === "number" ? value.toLocaleString("en-US") : value
  if (lang === "en") return str
  return str.replace(/[0-9]/g, (d) => faDigitMap[d] ?? d)
}

/**
 * Formats a percentage in each language's own convention: the "%" sign
 * comes before the number in Dari (e.g. "٪۸۰") and after it in English
 * (e.g. "80%").
 */
export function localizePercent(value: number, lang: Lang): string {
  const num = localizeNumber(value, lang)
  return lang === "fa" ? `%${num}` : `${num}%`
}
